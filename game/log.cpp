...
extern void SendCShieldLogNotification(const char* szText);

void LogManager::CShieldLog(LPCHARACTER ch, const DWORD errorcode, const bool kickPlayer, const TPacketCGHack data)
{
	if (!ch)
		return;
	...
	{
		if (kickPlayer && !IsErrorCodeInWhitelist(errorcode))
			d->SetPhase(PHASE_CLOSE);

		return;
	}

	char languageEscaped[3 * 2 + 1];
	char reportEscaped[1024 * 2 + 1];

	DBManager::instance().EscapeString(languageEscaped, sizeof(languageEscaped), data.language, strlen(data.language));
	DBManager::instance().EscapeString(reportEscaped, sizeof(reportEscaped), data.report, strlen(data.report));

	Query("INSERT INTO cshield_log%s (time, type, accountIds, playerName, level, mapIndex, x, y, ip, errorcode, why, win, language, report) "
		"VALUES(NOW(), %d, '%s', '%s', %d, %ld, %ld, %ld, '%s', %u, '%s', %d, '%s', '%s')",
		get_table_postfix(), 1, std::to_string(d->GetAccountTable().id).c_str(), ch->GetName(), ch->GetLevel(), ch->GetMapIndex(), ch->GetX(), ch->GetY(), d->GetHostName(), errorcode, MapErrorCode(errorcode).c_str(), data.win, languageEscaped, reportEscaped);

	ch->GetCShield()->AddErrorCodeToList(errorcode);

#ifdef ENABLE_LOG_NOTIFICATIONS
	if (notificationErrorCodes.find(errorcode) != notificationErrorCodes.end())
	{
		char messageBuf[CHAT_MAX_LEN + 1];
		snprintf(messageBuf, sizeof(messageBuf), "Player: %s; Where: %ld,%ld; Win: %d, Code: %u; %s", ch->GetName(), ch->GetX(), ch->GetY(), data.win, errorcode, MapErrorCode(errorcode).c_str());

		TPacketGGCShieldLogNotification p;
		p.bHeader = HEADER_GG_CSHIELD_LOG_NOTIFICATION;
		strlcpy(p.szText, messageBuf, sizeof(p.szText));
		P2P_MANAGER::instance().Send(&p, sizeof(p));
		SendCShieldLogNotification(messageBuf);
	}
#endif
	if (kickPlayer && !IsErrorCodeInWhitelist(errorcode))
		d->SetPhase(PHASE_CLOSE);
