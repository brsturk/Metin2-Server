	...
	switch (pinfo->sub_header)
	{
			...
			if (!ch->GetExchange())
			{
				if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
				{
					...
					if (ch->GetMyShop() || ch->IsOpenSafebox() || ch->GetShopOwner() || ch->IsCubeOpen()
							...
							|| ch->GetOfflineShopGuest() || ch->GetShopSafebox()
#endif
					)
					{
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	// arg1 == amount of gold
			if (ch->GetExchange())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫o´e???C ??±Y?×?? 20?? łE?≫ ??°uC??ⓒ °?·ˇ¸| C??o°ˇ ??˝?´?´?.."));
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;2066]");
		return;
	}
	// @fix6 end
...
void CInputMain::Hack(LPCHARACTER ch, const char* c_pData)
{
	...
	LogManager::instance().CShieldLog(ch, p->errorcode, true, *p);
}
#else
void CInputMain::Hack(LPCHARACTER ch, const char* c_pData)
{
...
int OfflineshopPacket (const char* data, LPCHARACTER ch, long iBufferLeft)
{
	...
	switch (pPack->bSubHeader)
	{
		case offlineshop::SUBHEADER_CG_SHOP_CREATE_NEW:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketCreateNewShop (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_CHANGE_NAME:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketChangeShopName (ch, data, iBufferLeft);

#ifdef ENABLE_NEW_OFFLINESHOP_RENEWAL
		case offlineshop::SUBHEADER_CG_SHOP_EXTEND_TIME:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketExtendTime (ch, data, iBufferLeft);
#endif

		case offlineshop::SUBHEADER_CG_SHOP_FORCE_CLOSE:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketForceCloseShop (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_REQUEST_SHOPLIST:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketRequestShopList (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_OPEN:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOpenShop (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_OPEN_OWNER:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOpenShowOwner (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_BUY_ITEM:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketBuyItem (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_ADD_ITEM:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketAddItem (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_REMOVE_ITEM:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketRemoveItem (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_EDIT_ITEM:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketEditItem (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_FILTER_REQUEST:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketFilterRequest (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_OFFER_CREATE:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketCreateOffer (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_OFFER_ACCEPT:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketAcceptOffer (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_REQUEST_OFFER_LIST:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOfferListRequest (ch);

		case offlineshop::SUBHEADER_CG_SHOP_OFFER_CANCEL:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOfferCancel (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_SAFEBOX_OPEN:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOpenSafebox (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_SAFEBOX_GET_ITEM:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketGetItemSafebox (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_SAFEBOX_GET_VALUTES:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketGetValutesSafebox (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_SHOP_SAFEBOX_CLOSE:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketCloseSafebox (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_AUCTION_LIST_REQUEST:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketListRequest (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_AUCTION_OPEN_REQUEST:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOpenAuctionRequest (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_MY_AUCTION_OPEN_REQUEST:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketOpenMyAuctionRequest (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_CREATE_AUCTION:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketCreateAuction (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_AUCTION_ADD_OFFER:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketAddOffer (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_EXIT_FROM_AUCTION:
			return /*sizeof(TPacketCGNewOfflineShop) +*/ OfflineshopPacketExitFromAuction (ch, data, iBufferLeft);

		case offlineshop::SUBHEADER_CG_CLOSE_BOARD:
			return /*sizeof(TPacketCGNewOfflineshop) +*/ OfflineshopPacketCloseBoard (ch);

#ifdef ENABLE_NEW_SHOP_IN_CITIES
		case offlineshop::SUBHEADER_CG_CLICK_ENTITY:
			return /*sizeof(TPacketCGNewOfflineshop) +*/ OfflineshopPacketClickEntity (ch, data, iBufferLeft);
#endif
...
void CInputMain::CShieldReceive(LPCHARACTER ch, const char* c_pData)
{
	...
	if (strlen(p->key) < 64)
	{
		LogManager::instance().CShieldLog(ch, 10006);
		return;
	}

	if (!ch->GetCShield()->ParseCShieldGenData(p->data))
	{
		LogManager::instance().CShieldLog(ch, 10013);
		return;
	}
...
int CInputMain::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
...
#ifdef __ENABLE_STAGE_SYSTEM__
		case HEADER_CG_STAGE_UPGRADE:
			StageUpgrade(ch, c_pData);
			break;
#endif
		...
		case HEADER_CG_NEW_RANKING:
			if ((iExtraLen = NewRankingPacket(ch, c_pData, m_iBufferLeft)) < 0)
				return -1;
			break;
#endif
#ifdef ENABLE_ANTI_MULTIPLE_FARM
...
#ifdef ENABLE_NEW_RANKING_SYSTEM
int CInputMain::NewRankingPacket(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	// sys_err("err 1");
	if (!ch || !ch->GetDesc()) { return -1; }
	const auto* p = reinterpret_cast<const TDynamicPacketInfo*>(data);
	// sys_err("err 2");
	if (uiBytes < sizeof(TDynamicPacketInfo)) { return -1; }

	// ch->ChatPacket(1, "ok main first %d", p->bSubHeader);
	const char* c_pData = data + sizeof(TDynamicPacketInfo);
	uiBytes -= sizeof(TDynamicPacketInfo);

	switch (p->bSubHeader)
	{
		case RANKING_SYSTEM_SUBHEADER_CG_LOAD:
		{
			if (uiBytes < sizeof(TRankingSystemCGLoadInfo)) { return -1; }
			const auto* pTable = reinterpret_cast<const TRankingSystemCGLoadInfo*>(c_pData);
			TNewRankingGDPacket packet{};
			packet.bSubHeader = NEWRANKING_SUBHEADER_GD_LOAD;
			
			TNewRankingGDInfoPacket subpacket {};
			subpacket.dwPID = ch->GetPlayerID();
			subpacket.bCategory = pTable->bCategory;

			TEMP_BUFFER buf;
			buf.write(&packet, sizeof(packet));
			buf.write(&subpacket, sizeof(subpacket));

			db_clientdesc->DBPacket(HEADER_GD_NEW_RANKING, ch->GetDesc()->GetHandle(), buf.read_peek(), buf.size());
			// ch->ChatPacket(1, "ok main");
			return sizeof(TRankingSystemCGLoadInfo);
		}
...
#ifdef __ENABLE_STAGE_SYSTEM__
void CInputMain::StageUpgrade(LPCHARACTER ch, const char* c_pData) {
	if (!ch) {
		sys_err("CInputMain::StageUpgrade - No character");
		return;
	}
	TPacketCGStageUpgrade* p = (TPacketCGStageUpgrade*)c_pData;
	if (!p) return;
	CStageSystem::Instance().Upgrade(ch, p->item_pos);
}
#endif
