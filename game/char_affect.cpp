...
#define IS_NO_CLEAR_ON_DEATH_AFFECT(type) ((type) == AFFECT_BLOCK_CHAT || ((type) >= 500 && (type) < 812))
...
#ifdef ENABLE_NEW_BIOLOG
			const auto maxSize = CNewBiologManager::instance().GetMissionSize();
			if (pkAff->dwType >= AFFECT_BIOLOG_START_IDX && pkAff->dwType <= AFFECT_BIOLOG_START_IDX + maxSize)
			{
				++it;
				continue;
			}
