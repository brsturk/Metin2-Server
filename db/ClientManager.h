...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	public:
	// void ResetRank(bool isWeekly);
	void UpdateRankingInfo();

	void RecvNewRankingGDInfo(CPeer* peer, DWORD dwHandle, const char* c_pData);
	void SendNewRankingGDInfoQuery(CPeer* peer, DWORD dwHandle, TNewRankingGDInfoPacket* packet);

	// void SendNewRankingRewardResultQuery(CPeer* peer, DWORD dwHandle, TNewRankingGDRewardPacket* packet);
	private:
	std::map<uint32_t, TRankingInfo> m_vecRankingInfo;
	TNewRankingInfo	m_kNewRankingInfo;
	std::map<uint32_t, std::vector<TNewRankingPlayerCachedinfo> > m_PlayerCacheInfos;
#endif
	private:
...
#ifdef ENABLE_CSHIELD
	public:
	CCShieldCache* GetCShieldCache(DWORD id);
	void PutCShieldCache(const TCShield* pNew);
	void UpdateCShieldCache();
#ifdef ENABLE_EXTERNAL_REPORTS
	CCShieldReportCache* GetCShieldReportCache(std::string cshieldHwid);
	void PutCShieldReportCache(const TCShieldReport* pNew);
	void UpdateCShieldReportCache();
#endif

	private:
	typedef std::unordered_map<DWORD, CCShieldCache*> TCShieldCacheMap;
	TCShieldCacheMap m_map_CShieldCache;

	void QUERY_CSHIELD_LOAD(CPeer* peer, DWORD dwHandle, TPlayerLoadPacket* packet);
	void RESULT_CSHIELD_LOAD(CPeer* peer, MYSQL_RES* pRes, DWORD dwHandle);
	void QUERY_CSHIELD_SAVE(const char* c_pData);

#ifdef ENABLE_EXTERNAL_REPORTS
	typedef std::unordered_map<std::string, CCShieldReportCache*> TCShieldReportCacheMap;
	TCShieldReportCacheMap m_map_CShieldReportCache;

	void QUERY_CSHIELD_REPORT_SAVE(const char* c_pData);
#endif
#endif
