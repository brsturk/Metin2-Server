...
enum
{
	...
	HEADER_GD_NEW_RANKING						= 142,
#endif
#ifdef __ENABLE_NEW_OFFLINESHOP__
	HEADER_GD_NEW_OFFLINESHOP = 153,
#endif
#ifdef ENABLE_CSHIELD
	HEADER_GD_CSHIELD_SAVE						= 150,
#ifdef ENABLE_EXTERNAL_REPORTS
	HEADER_GD_CSHIELD_REPORT_SAVE				= 151,
#endif
#endif
	...
	HEADER_DG_EVENT_MANAGER						= 237,
	HEADER_GD_EVENT_MANAGER						= 238,
#endif	
#ifdef ENABLE_NEW_RANKING_SYSTEM
	HEADER_DG_NEW_RANKING						= 239,
#endif
#ifdef ENABLE_CSHIELD
	HEADER_DG_CSHIELD_LOAD						= 240,
#endif
...
#ifdef ENABLE_NEW_RANKING_SYSTEM
typedef struct SPlayerRanking
{
	uint64_t 	ullStoneDamage;
	uint32_t	uiDestroyedStone;
	uint32_t	uiDestroyedBoss;
	uint32_t	uiUsedSwitcher;
	uint32_t	uiOpenedChest;
	uint32_t	uiComplatedDungeon;
	uint64_t	ullBossDamage;
	uint32_t	uibarones;
	uint32_t	uiejder;
	uint32_t	uiazrail;
	uint32_t	uilusifer;
	...
	uint32_t	uirazador;
	uint32_t	uinemere;
	uint64_t	ullplayerdamage;
	uint32_t	uiplayerkill;
} TPlayerRanking;
#endif
...
using TRankingInfo = struct SRankingInfo
{
...
using TRankingTopPlayersInfo = struct SRankingTopPlayersInfo
{
	TRankingInfo	aRankingInfo;
};

using TRankingPlayerInfo = struct SRankingPlayerInfo
{
	uint32_t 	rank;
	uint64_t 	value;
};

using TNewRankingPlayerCachedinfo = struct SNewRankingPlayerCachedinfo
{
	TRankingPlayerInfo aRankingPlayerInfo;
	bool isUpdate;
};


/* Game to Database */
using TNewRankingGDPacket = struct SNewRankingGDPacket
{
	uint8_t bSubHeader;
};

using TNewRankingGDInfoPacket = struct SNewRankingGDInfoPacket
{
	uint32_t dwPID;
	uint8_t bCategory;
};

using TNewRankingGDUpdatePacket = struct SNewRankingGDUpdatePacket
{
	uint8_t bSubHeader;
	uint32_t dwPID;
	uint8_t bCategory;
...
using rankingArr = std::array<TRankingTopPlayersInfo, RANKING_MAX_NUM>;

using TNewRankingInfo = struct SNewRankingInfo
{
	rankingArr aRankingLevelInfo;
	rankingArr aRankingMinutesInfo;
	rankingArr aRankingStoneDamageInfo;
	rankingArr aRankingStoneDestroyInfo;
	rankingArr aRankingBossDestroyInfo;
	rankingArr aRankingSwitcherInfo;
	rankingArr aRankingOpenChestInfo;
	rankingArr aRankingCompletedDungeonInfo;
	rankingArr aRankingBossDamageInfo;
	rankingArr aRankingBaronesInfo;
	rankingArr aRankingEjderInfo;
	rankingArr aRankingAzrailInfo;
	rankingArr aRankingLusiferInfo;
	rankingArr aRankingKirmiziInfo;
	rankingArr aRankingRazadorInfo;
	rankingArr aRankingNemereInfo;
	rankingArr aRankingElitOrkInfo;
	rankingArr aRankingPlayerDamageInfo;
	rankingArr aRankingPlayerKillInfo;
};

enum ENewRankingGDSubPackets : uint8_t 
{
	NEWRANKING_SUBHEADER_GD_LOAD,
	// NEWRANKING_SUBHEADER_GD_RESET,
	// NEWRANKING_SUBHEADER_GD_REWARD,
};

/* Database to Game */

enum ENewRankingDGSubPackets : uint8_t
{
	NEWRANKING_SUBHEADER_DG_LOAD,
	// NEWRANKING_SUBHEADER_DG_REWARD,
	// NEWRANKING_SUBHEADER_DG_RESET_RANKS,
};

using TNewRankingDGPacket = struct SNewRankingDGPacket
{
	uint8_t bSubHeader;
};

using TNewRankingDGRankingInfo = struct SNewRankingDGRankingInfo
{
	rankingArr aRankingInfo;
	TRankingPlayerInfo aRankingPlayerInfo;
};

// using TNewRankingRewardPacket = struct SNewRankingRewardPacket
// {
	// std::array<uint32_t, RANKING_REWARD_COUNT> aRewardsVnum;
	// std::array<uint32_t, RANKING_REWARD_COUNT> aRewardsCount;
// };
#endif
