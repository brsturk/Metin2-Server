...
ACMD (do_mob_coward) // @fix5 : negative number fix
{
	char arg1[256], arg2[256];
	DWORD vnum = 0;
	LPCHARACTER tch;
...
ACMD (do_mob_aggresive) // @fix5 : negatif number core
{
	char arg1[256], arg2[256];
	DWORD vnum = 0;
	LPCHARACTER tch;
...
ACMD (do_mob) // @fix5 : negative number core
{
	char arg1[256], arg2[256];
	DWORD vnum = 0;
	two_arguments (argument, arg1, sizeof (arg1), arg2, sizeof (arg2));
		...
		case DoSetTypes::GOLD:	// gold
			{
				...
				{
					long long difference = after_gold - GOLD_MAX; // full yang (def : int)
...
// #ifdef CHARACTER_COMMAND_RENEWAL
ACMD (do_advance)
{
	char arg1[256], arg2[256];
	two_arguments (argument, arg1, sizeof (arg1), arg2, sizeof (arg2));
	...
	unsigned char old_level = tch->GetLevel();
	...
	if (new_level < old_level)
	{
		//tch->ClearSkill();     // If you want to clear players current skills after the level downgrade, uncomment this line.
	...
	if (new_level < 50 && old_level >= 50) // Hard-coding for subskills, 50 because of the combo and horse skills.. 
	{
		tch->ClearSubSkill();  // If you don't want to clear players current subskills after the level downgrade under 50, comment this line. 
	}
}
// #endif
...
//ACMD (do_fishing_simul) // @fix5 : negative number core
//{
//	char arg1[256];
//	char arg2[256];
//	char arg3[256];
...
ACMD(do_fishing_simul) // @fix5 : negative number core
{
	// Simulasyon ozelli?i sadele?tirilmi? sistemde kaldırıldı
	ch->ChatPacket(CHAT_TYPE_INFO, "[LS;2089]");
	return;
}
...
ACMD (do_level)
{
	...
	unsigned char old_level = ch->GetLevel();
	unsigned char new_level = 0;
	...
	// This code use only allowed for GM chars, so we don't need to check or reset skills and subskills.. 
...
// #ifdef FULL_SET_COMMAND_RENEWAL
ACMD (do_all_skill_master) 
{
	...
	ch->SetHorseLevel (30); // Soldier Horse
	ch->ComputePoints(); // @fix300
	ch->SkillLevelPacket();
...
{
	if (ch->GetLevel() < 90) // 90 Lvl is enough for items that i use in this function, if you change vnums in the future, make sure change this level limit!
	{
		ch->ChatPacket (CHAT_TYPE_INFO, "<GM Control> You need to at least 90 Level for use this command!");
		return;
	}

	struct GMItemInfo // General structure for all job types. 
	{
		DWORD vnum;
		BYTE count;
	};

	LPITEM pItem; // We need to look all equipments and unequip all of them (Thanks to Ken for the idea).
	for (BYTE i = 0; i < WEAR_MAX_NUM; i++) // Using WEAR_MAX_NUM instead of hard-coded value.
	{
	...
	BYTE job = ch->GetJob();
	switch (job) // Ps; Item & Bonus names were written with reference to the Turkish locale..
	{
		case JOB_SURA:
		{
		...
		case JOB_WARRIOR:
		{
			const std::vector<GMItemInfo> Warrioritems =
			{
				...
				{13149, 1}, // Titan Kalkan +9          (WEAR_SHIELD)
				{15419, 1}, // Kirin Ayakkabi +9        (WEAR_FOOTS)
				{219,   1}, // Oldurucu Pala +9         (WEAR_WEAPON)
...
{
	BYTE job = ch->GetJob();
	switch (job) // I know SetForceAttribute use UpdatePacket for each time but this command only useable by few GM's so that's not a big deal.. 
	{
		case JOB_WARRIOR:
		case JOB_ASSASSIN:
		case JOB_SURA:
...
ACMD (do_full_set)
{
	if (ch->GetLevel() < 75) // DEV Note : Berat istedigi icin 75 e dusurdum normalde 90 - 21.04.2025
	{
		ch->ChatPacket (CHAT_TYPE_INFO, "<GM Control> You need to at least 75 Level for use this command!");
		return;
	}
...
ACMD (do_item_vs_set)
{
	...
	switch (job) // Ps; Item & Bonus names were written with reference to the Turkish locale..
	{
		case JOB_SURA:
		{
			const std::vector<GMItemInfo> Suraitems =
			{
				{11699, 1},	// Kara Buyu Zirh+9			(WEAR_BODY)
				{13109, 1},	// Aslan Kaplan+9			(WEAR_SHIELD)
				{15089, 1},	// Deri Cizme +9			(WEAR_FOOTS)
				{159,   1}, // Hortlak +9				(WEAR_WEAPON)
				{12539, 1}, // Boynuzlu Kask +9			(WEAR_HEAD)
				{14209, 1}, // Cennetin Gozu Bilezik +9	(WEAR_WRIST)
				{17209, 1}, // Cennetin Gozu Kupe +9	(WEAR_EAR)
				{16129, 1}, // inci kolye +9			(WEAR_NECK)
				{72702, 1}, // Ruzgarin Ayakkabilari	(WEAR_UNIQUE1)
			};
		...
		case JOB_WARRIOR:
		{
			const std::vector<GMItemInfo> Warrioritems =
			{
				{11299, 1}, // Siyah Celik Zirh+9		(WEAR_BODY)
				{13069, 1}, // Sahin Kalkan +9			(WEAR_SHIELD)
				{15089, 1}, // Deri Cizme +9			(WEAR_FOOTS)
				{149,   1}, // Muharebe +9				(WEAR_WEAPON)
				{12269, 1}, // Korku Maskesi +9			(WEAR_HEAD)
				{14209, 1}, // Cennetin Gozu Bilezik +9	(WEAR_WRIST)
				{17109, 1}, // Abanoz Kupe +9			(WEAR_EAR)
				{16209, 1}, // Cennetin Gozu Kolye +9	(WEAR_NECK)
		...
		case JOB_SHAMAN:
		{
			const std::vector<GMItemInfo> Shamanitems =
			{
				{11899, 1}, // Siyah Elbise				(WEAR_BODY)
				{13129, 1}, // Ejderha Pulu +9			(WEAR_SHIELD)
				{15089, 1}, // Deri Cizme +9			(WEAR_FOOTS)
				{7149,  1}, // Kurtulus Yelpazesi +9	(WEAR_WEAPON)
				{12539, 1}, // Boynuzlu Kask +9			(WEAR_HEAD)
				{14209, 1}, // Cennetin Gozu Bilezik +9	(WEAR_WRIST)
				{17209, 1}, // Cennetin Gozu Kupe +9	(WEAR_EAR)
				{16129, 1}, // inci kolye +9			(WEAR_NECK)
		...
		case JOB_ASSASSIN:
		{
			const std::vector<GMItemInfo> Assassinitems =
			{
				{11499, 1}, // Siyah Ruzgar+9			(WEAR_BODY)
				{13089, 1}, // Kaplan Kalkan +9			(WEAR_SHIELD)
				{15089, 1}, // Deri Cizme +9			(WEAR_FOOTS)
				{1109,  1}, // Ejderha Bicagi +9		(WEAR_WEAPON)
				{12399, 1}, // Ork Kapsonu +9			(WEAR_HEAD)
				{14209, 1}, // Cennetin Gozu Bilezik +9	(WEAR_WRIST)
				{17109, 1}, // Abanoz Kupe +9			(WEAR_EAR)
				{16209, 1}, // Cennetin Gozu Kolye +9	(WEAR_NECK)
	...
	switch (job) // I know SetForceAttribute use UpdatePacket for each time but this command only useable by few GM's so that's not a big deal.. 
	{
		...
		case JOB_WARRIOR:
			...
			if (item)
			{
				...
				item->SetForceAttribute (4, APPLY_POISON_PCT,		8);
				ch->ChatPacket (CHAT_TYPE_INFO, "<GM> Weapon attr is changed successfully..");
			}
			else
			{
			...
			if (item)
			{
				...
				item->SetForceAttribute (2, APPLY_IMMUNE_STUN,		1);
				item->SetForceAttribute (3, APPLY_ATTBONUS_HUMAN,	10);
				item->SetForceAttribute (4, APPLY_BLOCK,			15);
				ch->ChatPacket (CHAT_TYPE_INFO, "<GM> Shield attr is changed successfully..");
			}
			else
			{
			...
			if (item)
			{
				item->ClearAttribute();
				item->SetForceAttribute (0, APPLY_ATTBONUS_HUMAN,	10);
				item->SetForceAttribute (1, APPLY_CAST_SPEED ,		15);
				item->SetForceAttribute (2, APPLY_ATT_SPEED ,		8);
			...
			if (item)
			{
				...
				item->SetForceAttribute (2, APPLY_RESIST_TWOHAND,	15);
				item->SetForceAttribute (3, APPLY_RESIST_DAGGER,	15);
			...
			if (item)
			{
				...
				item->SetForceAttribute (4, APPLY_CAST_SPEED,			15);
				ch->ChatPacket (CHAT_TYPE_INFO, "<GM> Wrist attr is changed successfully..");
			}
			else
			{
...
{
	do_all_skill_master (ch, NULL, 0, 0);
	do_item_vs_set (ch, NULL, 0, 0);
	do_attr_vs_set (ch, NULL, 0, 0);
	ch->ChatPacket (CHAT_TYPE_INFO, "<GM> Everything for the Duel is done!");
}

// VS EKIPMAN COMMAND END
...
ACMD(do_captcha)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: Usage: /captcha <name>");
		return;
	}
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			if (pkCCI->bChannel != g_bChannel)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: Target (%s) is in %d channel (my channel %d)", arg1, pkCCI->bChannel, g_bChannel);
				return;
			}
		}
		else
			ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: There is no character (%s) by that name", arg1);

		ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: Target (%s) is not in the same map with you!", arg1);
		return;
	}

	int chMapIdx = ch->GetMapIndex();
	int tchMapIdx = tch->GetMapIndex();
	if (chMapIdx != tchMapIdx)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: Target (%s) is not in the same map with you!", arg1);
		return;
	}
	...
	if (ch == tch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: Cannot captcha yourself.");
		return;
	}

	tch->ResetCShieldCaptcha();
	ch->ChatPacket(CHAT_TYPE_INFO, "[CAPTCHA]: Captcha challenge requested succesfully!");
}
#endif
