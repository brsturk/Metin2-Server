...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	if (IsPC())
	{
		...
		DWORD result = 0;
#ifdef ENABLE_EXTENDED_CHECK_ATTACKSPEED_HACK
		bool extendedCheck = true;
#else
		bool extendedCheck = false;
#endif

		if (IsPolymorphed())
		{
			if (pkVictim->IsMonster() && pkVictim->GetMobRank() >= MOB_RANK_BOSS)
				result = GetCShield()->CheckAttackspeedHack(false, false, false, (DWORD)(CMotionManager::instance().GetNormalAttackDuration(GetRaceNum()) * 700.0f), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 1, extendedCheck);
			else if (pkVictim->IsMonster())
				result = GetCShield()->CheckAttackspeedHack(false, false, false, (DWORD)(CMotionManager::instance().GetNormalAttackDuration(GetRaceNum()) * 700.0f), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 2, extendedCheck);
			else if (pkVictim->IsStone())
				result = GetCShield()->CheckAttackspeedHack(false, false, false, (DWORD)(CMotionManager::instance().GetNormalAttackDuration(GetRaceNum()) * 700.0f), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 3, extendedCheck);
			else if (pkVictim->IsPC())
				result = GetCShield()->CheckAttackspeedHack(false, false, false, (DWORD)(CMotionManager::instance().GetNormalAttackDuration(GetRaceNum()) * 700.0f), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 4, extendedCheck);
		}
		else
		{
			if (pkVictim->IsMonster() && pkVictim->GetMobRank() >= MOB_RANK_BOSS)
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 1, extendedCheck);
			else if (pkVictim->IsMonster())
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 2, extendedCheck);
			else if (pkVictim->IsStone())
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 3, extendedCheck);
			else if (pkVictim->IsPC())
				result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time(), 4, extendedCheck);
		}

		if (result == 1)
		{
#ifdef DISABLE_ATTACKSPEED_KICK
			LogManager::instance().CShieldLog(this, 10005, false);
#else
			LogManager::instance().CShieldLog(this, 10005);
#endif
			return false;
		}
		else if (result == 2)
...
{
	// ADD_PREMIUM
	[[maybe_unused]] bool isAutoLoot =
		(pkAttacker->GetPremiumRemainSeconds(PREMIUM_AUTOLOOT) > 0 ||
		 pkAttacker->IsEquipUniqueGroup(UNIQUE_GROUP_AUTOLOOT))
		? true : false;
	// END_OF_ADD_PREMIUM

	[[maybe_unused]] PIXEL_POSITION pos = GetXYZ(); // @fixme194 GetMovablePosition is useless here
	...
	if (pkAttacker->IsPC())
	{
		const auto event = CHARACTER_MANAGER::Instance().CheckEventIsActive(YANG_EVENT, pkAttacker->GetEmpire());
		if(event != NULL)
...
void CHARACTER::Reward(bool bItemDrop)
{
	if (GetRaceNum() == 5001)
	{
		...
		for (int i = 1; i <= iSplitCount; ++i)
		{
			if ((item = ITEM_MANAGER::instance().CreateItem(1, iGold / iSplitCount)))
			{
				if (i != 0)
				{
					pos.x = number(-7, 7) * 20 + GetX();
					pos.y = number(-7, 7) * 20 + GetY();
				}
	...
	if (ITEM_MANAGER::instance().CreateDropItem(this, pkAttacker, s_vec_item))
	{
		...
		{
			sys_log(0, "No items dropped for %s", pkAttacker->GetName());
			return;
		}

		// Grup varsa, CanPickupDirectly() acık bir oyuncu var mı kontrol et
		bool bAnyPickupEnabled = false;
		if (pkAttacker->GetParty())
		{
			CParty* pParty = pkAttacker->GetParty();
			if (pParty)
			{
				auto pickupChecker = [&](LPCHARACTER ch) {
					if (ch && ch->CanPickupDirectly())
					{
						sys_log(0, "CAN_PICKUP_CHECK: %s has CanPickupDirectly enabled", ch->GetName());
						bAnyPickupEnabled = true;
						return false; // ?lk acık oyuncuyu bulunca durdur
					}
					sys_log(0, "CAN_PICKUP_CHECK: %s has CanPickupDirectly disabled", ch->GetName());
					return true;
				};
				pParty->ForEachMemberPtr(pickupChecker);
			}
		}
		else
		{
			// Solo oyuncu icin pkAttacker kontrol edilir
			if (pkAttacker->CanPickupDirectly())
			{
				bAnyPickupEnabled = true;
				sys_log(0, "CAN_PICKUP_CHECK: %s has CanPickupDirectly enabled (solo)", pkAttacker->GetName());
			}
			else
			{
				sys_log(0, "CAN_PICKUP_CHECK: %s has CanPickupDirectly disabled (solo)", pkAttacker->GetName());
			}
		}

		// m_map_kDamage loglama (kontrol icin)
		for (auto it = m_map_kDamage.begin(); it != m_map_kDamage.end(); ++it)
		{
			if (it->second.iTotalDamage > 0)
			{
				...
				if (ch)
				{
					sys_log(0, "DAMAGE_LOG: %s dealt %d damage", ch->GetName(), it->second.iTotalDamage);
				}
			}
		}

		// Itemleri hasar veren oyunculara da?ıt
		std::vector<LPCHARACTER> damageContributors;
		if (pkAttacker && m_map_kDamage.count(pkAttacker->GetVID()) && m_map_kDamage[pkAttacker->GetVID()].iTotalDamage > 0 &&
			DISTANCE_APPROX(pkAttacker->GetX() - pos.x, pkAttacker->GetY() - pos.y) <= 5000)
		{
			damageContributors.push_back(pkAttacker);
			sys_log(0, "AUTO_PICKUP: %s added to damage contributors (attacker, Damage=%d, Distance=%ld)",
				pkAttacker->GetName(), m_map_kDamage[pkAttacker->GetVID()].iTotalDamage,
				DISTANCE_APPROX(pkAttacker->GetX() - pos.x, pkAttacker->GetY() - pos.y));
		}
		else if (pkAttacker)
		{
			sys_log(0, "AUTO_PICKUP: %s not eligible as damage contributor (Damage=%d, Distance=%ld)",
				pkAttacker->GetName(), m_map_kDamage.count(pkAttacker->GetVID()) ? m_map_kDamage[pkAttacker->GetVID()].iTotalDamage : 0,
				DISTANCE_APPROX(pkAttacker->GetX() - pos.x, pkAttacker->GetY() - pos.y));
		}
		...
		if (pkAttacker->GetParty())
		{
			...
			if (pParty)
			{
				for (auto it = m_map_kDamage.begin(); it != m_map_kDamage.end(); ++it)
				{
					...
					{
						LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(it->first);
						if (ch && ch->GetParty() == pParty && ch != pkAttacker &&
							DISTANCE_APPROX(ch->GetX() - pos.x, ch->GetY() - pos.y) <= 5000)
						{
							damageContributors.push_back(ch);
							sys_log(0, "AUTO_PICKUP: %s added to damage contributors (group, Damage=%d, Distance=%ld)",
								ch->GetName(), it->second.iTotalDamage,
								DISTANCE_APPROX(ch->GetX() - pos.x, ch->GetY() - pos.y));
						}
						else if (ch)
						{
							sys_log(0, "AUTO_PICKUP: %s not eligible as damage contributor (group, Damage=%d, Distance=%ld)",
								ch->GetName(), it->second.iTotalDamage,
								DISTANCE_APPROX(ch->GetX() - pos.x, ch->GetY() - pos.y));
						}
					}
		...
		// Tum itemleri i?le
		for (size_t i = 0; i < s_vec_item.size(); ++i)
		{
			item = s_vec_item[i];
			if (!item)
			{
				sys_err("item null in vector idx %zu", i);
				continue;
			}

#ifdef RENEWAL_PICKUP_AFFECT
			if (bAnyPickupEnabled && !damageContributors.empty())
			{
				// Itemleri hasar veren oyuncular arasında sırayla da?ıt
				LPCHARACTER targetCh = damageContributors[i % damageContributors.size()];
				targetCh->AutoGiveItemNew(item);
				sys_log(0, "AUTO_PICKUP: %s to %s (damage contributor, item %zu)", item->GetName(), targetCh->GetName(), i);
				continue;
			}
			else if (bAnyPickupEnabled)
			{
				sys_log(0, "AUTO_PICKUP: No eligible damage contributors found for item %s", item->GetName());
			}
#endif

			// Uygun oyuncu yoksa veya otomatik toplama kapalıysa yere du?ur
			item->AddToGround(GetMapIndex(), pos);
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	{
		if (!pkKiller->GetDungeon() && IsMonster() && GetMobRank() >= MOB_RANK_BOSS)
		{
			pkKiller->GetCShield()->SetBossCount(pkKiller->GetCShield()->GetBossCount() + 1);

			if (pkKiller->GetCShield()->GetBossCount() >= 15)
			{
				pkKiller->GetCShield()->SetBossCount(0);
				LogManager::instance().CShieldLog(pkKiller, 10011, false);
			}
		}
		else if (IsStone())
		{
			pkKiller->GetCShield()->SetMetinCount(pkKiller->GetCShield()->GetMetinCount() + 1);

			if (pkKiller->GetCShield()->GetMetinCount() >= 25)
			{
				pkKiller->GetCShield()->SetMetinCount(0);
				LogManager::instance().CShieldLog(pkKiller, 10012, false);
			}
		}
	...
	if(!IsPC() && GetRaceNum())
	{
		...
		{
			pkKiller->SetBarones(pkKiller->GetBarones()+1);
		}
		else if (pkKiller && pkKiller->IsPC() && GetRaceNum() == 2493)
		{
			pkKiller->SetEjder(pkKiller->GetEjder()+1);
		}
		else if (pkKiller && pkKiller->IsPC() && GetRaceNum() == 2598)
		{
			pkKiller->SetAzrail(pkKiller->GetAzrail()+1);
		}
		else if (pkKiller && pkKiller->IsPC() && GetRaceNum() == 1093)
		{
			pkKiller->SetLusifer(pkKiller->GetLusifer()+1);
		}
		else if (pkKiller && pkKiller->IsPC() && GetRaceNum() == 2291)
		{
			pkKiller->SetKirmizi(pkKiller->GetKirmizi()+1);
		}
		else if (pkKiller && pkKiller->IsPC() && GetRaceNum() == 6091)
		{
			pkKiller->SetRazador(pkKiller->GetRazador()+1);
		}
		else if (pkKiller && pkKiller->IsPC() && GetRaceNum() == 6191)
		{
			pkKiller->SetNemere(pkKiller->GetNemere()+1);
		}
	}
#endif
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		{
			pkKiller->SetDestroyedStone(pkKiller->GetDestroyedStone()+1);
		}
		else if(IsBoss())
		{
			pkKiller->SetDestroyedBoss(pkKiller->GetDestroyedBoss()+1);
		}
#endif
	}
...
#endif
#if defined(ENABLE_ANTI_MULTIPLE_FARM) && defined(ENABLE_WORLDBOSS)
	if (IsWbMonster() && pAttacker && pAttacker->HasBlockedDrops())
	{
		SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
		return false;
	}
...
//			if (IsStone())
//			{
//				if (dam > pAttacker->GetStoneDamage())
//				{
//					pAttacker->SetStoneDamage(dam);
//				}
//			}
//			else if (IsBoss())
//			{
//				if (dam > pAttacker->GetBossDamage())
//				{
//					pAttacker->SetBossDamage(dam);
//				}
//			}
//			else if (IsPC())
//			{
//				if (dam > pAttacker->GetPlayerDamage())
//				{
//					pAttacker->SetPlayerDamage(dam);
//				}
//			}
//		}
//#endif
		...
		if (pAttacker)
		{
			// Sadece duz vuru?lar dahil edilsin (ta? ve boss icin)
			if ((type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE))
			{
				if (IsStone())
				{
			...
			// Oyuncuya vurulan her turlu hasar (duz ve skill) dahil edilsin
			if (IsPC())
			{
				if (dam > pAttacker->GetPlayerDamage())
					pAttacker->SetPlayerDamage(dam);
...
void CHARACTER::UseArrow(LPITEM pkArrow, DWORD dwArrowCount)
{
	...
	//iCount = iCount - MIN(iCount, dwArrowCount); - DEV NOTE : Arrows made unlimited..
				...
				case 0:
					{
						...
						if (m_me->IsPC())
						{
							...
							bool extendedCheck = false;
#endif
							DWORD result = m_me->GetCShield()->CheckAttackspeedBowHack(m_me->IsPolymorphed() ? (DWORD)(CMotionManager::instance().GetNormalAttackDuration(m_me->GetRaceNum()) * 700.0f) : ani_attack_speed(m_me), m_me->GetLimitPoint(POINT_ATT_SPEED), get_dword_time(), extendedCheck);
							if (result == 1)
							{
#ifdef DISABLE_ATTACKSPEED_KICK
								LogManager::instance().CShieldLog(m_me, 10005, false);
#else
								LogManager::instance().CShieldLog(m_me, 10005);
#endif
								iDam = 0;
							}
							else if (result == 2)
...
#endif
#ifdef ENABLE_CAPTCHA
							if (m_me->IsPC() && (pkVictim->IsMonster() || pkVictim->IsStone()))
							{
								if (m_me->CShieldCaptcha(10007))
									iDam = 0;
							}
#endif
...
struct FuncAggregateMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (number(1, 100) <= 70) // @fix2 (def 50)
