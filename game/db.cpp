		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
					...
					std::string loginKey = GetCShieldLoginKey(pinfo->login);
#endif
					if (!row[col])
					{
					...
					else if (loginKey.compare(0, loginKey.length(), pinfo->cshieldKey) != 0 && !IsAccountWhitelisted(pinfo->login))
#else
					else if (loginKey.compare(0, loginKey.length(), pinfo->cshieldKey) != 0)
#endif
					{
						LoginFailure(d, "CSHIELD");
						sys_log(0, "CSHIELD: Name: %s, Client(%d): %s, Server(%d): %s", pinfo->login, strlen(pinfo->cshieldKey), pinfo->cshieldKey, loginKey.length(), loginKey.c_str());
						M2_DELETE(pinfo);
						return;
					}
#ifdef ENABLE_CAPTCHA_AND_DEV_WHITELIST
					else if ((!VerifyHwid(pinfo->cshieldHwid) || IsHwidBlacklisted(pinfo->cshieldHwid)) && !IsAccountWhitelisted(pinfo->login))
#else
					else if (!VerifyHwid(pinfo->cshieldHwid) || IsHwidBlacklisted(pinfo->cshieldHwid))
#endif
					{
						LoginFailure(d, "CS_HWID");
						sys_log(0, "CSHIELD: Hwid: %s, Verified: %d", pinfo->cshieldHwid, VerifyHwid(pinfo->cshieldHwid));
						M2_DELETE(pinfo);
						return;
					}
#endif
...
#ifdef ENABLE_CSHIELD
							char szCShieldHwid[128 * 2 + 1];
							DBManager::instance().EscapeString(szCShieldHwid, sizeof(szCShieldHwid), pinfo->cshieldHwid, strlen(pinfo->cshieldHwid));
	
							char szCShieldQuery[1024];
							snprintf(szCShieldQuery, sizeof(szCShieldQuery), "UPDATE account SET cshield_hwid='%s' WHERE id=%u", szCShieldHwid, dwID);
							std::unique_ptr<SQLMsg> msgCShield(DBManager::instance().DirectQuery(szCShieldQuery));
#endif
							char szQuery[1024];
