...
bool CInputProcessor::Process(LPDESC lpDesc, const void * c_pvOrig, int iBytes, int & r_iBytesProceed)
{
	...
	for (m_iBufferLeft = iBytes; m_iBufferLeft > 0;)
	{
		...
		if (bHeader == HEADER_CG_CSHIELD_DATA)
		{
			c_pData	+= iPacketLen;
...
int CInputHandshake::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	if (bHeader == HEADER_CG_CSHIELD_DATA)
	{
		TPacketCGCShieldData* p = (TPacketCGCShieldData*)c_pData;
		if (!p)
		{
			d->SetPhase(PHASE_CLOSE);
			return 0;
		...
		std::string data = BuildCShieldResponse(p->version, p->binary_name, p->binary_hash);
		if (data.length() > 10)
		{
			d->Packet(data.c_str(), data.length());
#if defined(WHITELIST_IP) && !defined(_WIN32)
			WhitelistIP(d->GetHostName());
#endif
			d->DelayedDisconnect(15);
		}
		else
			d->SetPhase(PHASE_CLOSE);
	...
	if (bHeader == HEADER_CG_CSHIELD_REPORT)
	{
		TPacketCGCShieldReport* p = (TPacketCGCShieldReport*)c_pData;
		if (!p)
		{
			d->SetPhase(PHASE_CLOSE);
			return 0;
		...
		std::string ip = inet_ntoa(d->GetAddr().sin_addr);

		if (IsIpThrottled(ip, currentTime))
		{
			d->SetPhase(PHASE_CLOSE);
			return 0;
		}

		AddIpToThrottleList(ip, currentTime);

		TCShieldReport tab = { 0 };

		strlcpy(tab.ip, ip.c_str(), sizeof(tab.ip));
		tab.errorcode = p->errorcode;
		strlcpy(tab.why, MapErrorCode(p->errorcode).c_str(), sizeof(tab.why));
		tab.win = p->win;
		strlcpy(tab.language, p->language, sizeof(tab.language));
		strlcpy(tab.cshieldHwid, p->cshieldHwid, sizeof(tab.cshieldHwid));
		strlcpy(tab.report, p->report, sizeof(tab.report));

		db_clientdesc->DBPacketHeader(HEADER_GD_CSHIELD_REPORT_SAVE, 0, sizeof(TCShieldReport));
		db_clientdesc->Packet(&tab, sizeof(TCShieldReport));
