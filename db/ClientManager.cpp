...
void CClientManager::MainLoop()
{
	...
	TCShieldCacheMap::iterator itCShield = m_map_CShieldCache.begin();

	while (itCShield != m_map_CShieldCache.end())
	{
		CCShieldCache* pCache = itCShield->second;
		pCache->Flush();
		delete pCache;
		m_map_CShieldCache.erase(itCShield++);
	}

	m_map_CShieldCache.clear();

#ifdef ENABLE_EXTERNAL_REPORTS
	TCShieldReportCacheMap::iterator itCShieldReport = m_map_CShieldReportCache.begin();

	while (itCShieldReport != m_map_CShieldReportCache.end())
	{
		CCShieldReportCache* pCache = itCShieldReport->second;
		pCache->Flush();
		delete pCache;
		m_map_CShieldReportCache.erase(itCShieldReport++);
	}

	m_map_CShieldReportCache.clear();
#endif
#endif
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
...
#ifdef ENABLE_CSHIELD
				QUERY_CSHIELD_LOAD(peer, dwHandle, (TPlayerLoadPacket*)data);
#endif
				break;
			...
			case HEADER_GD_NEW_RANKING:
				RecvNewRankingGDInfo(peer, dwHandle, data);
				break;
#endif

#ifdef ENABLE_CSHIELD
			case HEADER_GD_CSHIELD_SAVE:
				QUERY_CSHIELD_SAVE(data);
				break;

#ifdef ENABLE_EXTERNAL_REPORTS
			case HEADER_GD_CSHIELD_REPORT_SAVE:
				QUERY_CSHIELD_REPORT_SAVE(data);
				break;
#endif
#endif
...
#ifdef ENABLE_CSHIELD
		case QID_CSHIELD_SAVE:
#ifdef ENABLE_EXTERNAL_REPORTS
		case QID_CSHIELD_REPORT_SAVE:
#endif
#endif
			break;
...
#ifdef ENABLE_NEW_RANKING_SYSTEM
		if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * 60 * RANKING_UPDATE_TIME)))
		{
#ifdef ENABLE_NEW_RANKING_SYSTEM_LOG
			sys_err("RANKING UPDATE OK : [%d]", thecore_heart->pulse);
#endif
			CClientManager::instance().UpdateRankingInfo();
		}
#endif
...
#ifdef ENABLE_CSHIELD
void CClientManager::QUERY_CSHIELD_SAVE(const char* c_pData)
{
	PutCShieldCache((TCShield*)c_pData);
}
CCShieldCache* CClientManager::GetCShieldCache(DWORD id)
{
	TCShieldCacheMap::iterator it = m_map_CShieldCache.find(id);
	
	if (it == m_map_CShieldCache.end())
		return NULL;

	return it->second;
}
void CClientManager::PutCShieldCache(const TCShield* pNew)
{
	CCShieldCache* pCache = GetCShieldCache(pNew->playerId);

	if (!pCache)
	{
		pCache = new CCShieldCache;
		m_map_CShieldCache.insert(TCShieldCacheMap::value_type(pNew->playerId, pCache));
	}

	pCache->Put(const_cast<TCShield*>(pNew), false);
}
void CClientManager::UpdateCShieldCache()
{
	TCShieldCacheMap::iterator it = m_map_CShieldCache.begin();

	while (it != m_map_CShieldCache.end())
	{
		CCShieldCache* pCache = it->second;
	
		if (pCache->CheckFlushTimeout())
		{
			pCache->Flush();
			delete pCache;
			m_map_CShieldCache.erase(it++);
		}
		else
			++it;
	}
...
#ifdef ENABLE_EXTERNAL_REPORTS
void CClientManager::QUERY_CSHIELD_REPORT_SAVE(const char* c_pData)
{
	PutCShieldReportCache((TCShieldReport*)c_pData);
}

CCShieldReportCache* CClientManager::GetCShieldReportCache(std::string cshieldHwid)
{
	TCShieldReportCacheMap::iterator it = m_map_CShieldReportCache.find(cshieldHwid);

	if (it == m_map_CShieldReportCache.end())
		return NULL;

	return it->second;
}

void CClientManager::PutCShieldReportCache(const TCShieldReport* pNew)
{
	CCShieldReportCache* pCache = GetCShieldReportCache(pNew->cshieldHwid);

	if (!pCache)
	{
		pCache = new CCShieldReportCache;
		m_map_CShieldReportCache.insert(TCShieldReportCacheMap::value_type(pNew->cshieldHwid, pCache));
	}

	pCache->Put(const_cast<TCShieldReport*>(pNew), false);
}

void CClientManager::UpdateCShieldReportCache()
{
	TCShieldReportCacheMap::iterator it = m_map_CShieldReportCache.begin();

	while (it != m_map_CShieldReportCache.end())
	{
		CCShieldReportCache* pCache = it->second;

		if (pCache->CheckFlushTimeout())
		{
			pCache->Flush();
			delete pCache;
			m_map_CShieldReportCache.erase(it++);
		}
		else
			++it;
	}
