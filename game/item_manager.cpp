...
bool ITEM_MANAGER::CreateDropItemVector(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<std::pair<int, int> >& vec_item)
{
	...
	// Drop Item Group
	{
		...
		if (it != m_map_pkDropItemGroup.end())
		{
			...
			{
				TItemTable* table = GetTable(v[i].dwVnum);
				...
				{
					if (table->dwVnum != pkChr->GetPolymorphItemVnum())
					{
						continue;
					}
				}

				vec_item.push_back(std::make_pair(table->dwVnum, v[i].iCount));
			}
		}
	}
	...
	if (pkChr->IsStone())
	{
		for (int i = 0; i < 14; ++i)
		{
			vec_item.push_back(std::make_pair(28030 + i, 1));
		}
	}
...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	[[maybe_unused]] int bonus = 0;
	const auto event = CHARACTER_MANAGER::Instance().CheckEventIsActive(DOUBLE_ITEM_EVENT, pkKiller->GetEmpire());
	if (event != NULL)
		bonus += event->value[0];
#endif
	...
	iRandRange = iRandRange * 100 /
		(100 +
		 ...
		 (pkKiller->IsEquipUniqueItem(UNIQUE_ITEM_DOUBLE_ITEM)?50:0)); // default (100:0)
