...
#ifdef __ENABLE_NEW_OFFLINESHOP__
			|| victim->GetOfflineShopGuest() || victim->GetShopSafebox()
#endif
	)
	{
...
bool CExchange::AddItem(TItemPos item_pos, BYTE display_pos)
{
	...
	if (item->IsSealed())
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;2060]");
		return false;
	}
#endif
...
bool CExchange::CheckSpace()
{
	...
	static CGrid s_grid5(SKILL_BOOK_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid6(SKILL_BOOK_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid7(SKILL_BOOK_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid8(SKILL_BOOK_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);

	static CGrid s_grid9(UPGRADE_ITEMS_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid10(UPGRADE_ITEMS_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid11(UPGRADE_ITEMS_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid12(UPGRADE_ITEMS_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);

	static CGrid s_grid13(STONE_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid14(STONE_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid15(STONE_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid16(STONE_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);

	static CGrid s_grid17(GIFT_BOX_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid18(GIFT_BOX_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid19(GIFT_BOX_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
	static CGrid s_grid20(GIFT_BOX_INVENTORY_MAX_NUM, SPECIAL_INVENTORY_PAGE_COUNT);
#endif
	...
	for (x = 0; x < UPGRADE_ITEMS_INVENTORY_MAX_NUM; ++x)
	{
		...
		if (x < SPECIAL_INVENTORY_PAGE_SIZE)
			s_grid9.Put(x, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 2)
			s_grid10.Put(x - SPECIAL_INVENTORY_PAGE_SIZE, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 3)
			s_grid11.Put(x - SPECIAL_INVENTORY_PAGE_SIZE * 2, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 4)
			s_grid12.Put(x - SPECIAL_INVENTORY_PAGE_SIZE * 3, 1, item->GetSize());
	}
	...
	for (x = 0; x < STONE_INVENTORY_MAX_NUM; ++x)
	{
		...
		if (x < SPECIAL_INVENTORY_PAGE_SIZE)
			s_grid13.Put(x, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 2)
			s_grid14.Put(x - SPECIAL_INVENTORY_PAGE_SIZE, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 3)
			s_grid15.Put(x - SPECIAL_INVENTORY_PAGE_SIZE * 2, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 4)
			s_grid16.Put(x - SPECIAL_INVENTORY_PAGE_SIZE * 3, 1, item->GetSize());
	}
	...
	for (x = 0; x < GIFT_BOX_INVENTORY_MAX_NUM; ++x)
	{
		...
		if (x < SPECIAL_INVENTORY_PAGE_SIZE)
			s_grid17.Put(x, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 2)
			s_grid18.Put(x - SPECIAL_INVENTORY_PAGE_SIZE, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 3)
			s_grid19.Put(x - SPECIAL_INVENTORY_PAGE_SIZE * 2, 1, item->GetSize());
		else if (x < SPECIAL_INVENTORY_PAGE_SIZE * 4)
			s_grid20.Put(x - SPECIAL_INVENTORY_PAGE_SIZE * 3, 1, item->GetSize());
	}
	// END_OF_GIFT_BOX
#endif
	...
	for (i = 0; i < EXCHANGE_ITEM_MAX_NUM; ++i)
	{
		...
		else if (item->IsUpgradeItem())
		{
			...
			if ((iPos = s_grid9.FindBlank(1, item->GetSize())) >= 0)
				s_grid9.Put(iPos, 1, item->GetSize());
			else if ((iPos = s_grid10.FindBlank(1, item->GetSize())) >= 0)
		...
		else if (item->IsGiftBox())
		{
			...
			if ((iPos = s_grid17.FindBlank(1, item->GetSize())) >= 0)
			...
			else if ((iPos = s_grid20.FindBlank(1, item->GetSize())) >= 0)
