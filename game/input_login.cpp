...
#ifdef ENABLE_ANTI_MULTIPLE_FARM
	d->SetLoginMacAdress(pinfo->cMAIf); // mac address sniffing 
#endif
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
...
#ifdef __ENABLE_STAGE_SYSTEM__
	CStageSystem::Instance().CheckStage(ch);			// Sure doldu mu kontrol
	CStageSystem::Instance().ApplyStageAffects(ch);		// Affect'leri temizle-yukle
#endif
	...
	if (!IsAccountWhitelisted(d->GetAccountTable().login))
	{
	...
	CHARACTER_MANAGER::Instance().CheckBackDungeon(ch, BACK_DUNGEON_CHECK);
#endif
