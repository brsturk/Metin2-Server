	...
	//offlineshop_shop_items
	{
		...
		if (pMsg->Get())
		{
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				str_to_number(item.item.iEvolution, row[col++]);
#endif
				//patch 08-03-2020
				BYTE bExpiration = 0;
...
#endif
#ifdef ENABLE_WEAPON_EVOLUTION_SYSTEM
				str_to_number(info.item.iEvolution, row[col++]);
#endif
				//patch 08-03-2020
				BYTE bexp = 0;
...
bool CClientManager::RecvOfflineShopBuyItemPacket(const char* data)
{
	...
	offlineshop::CShopCache::TShopCacheInfo* pCache =NULL;
	...
	if (!offerIDs.empty())
	{
		...
		{
			offlineshop::COfferCache::TOfferCacheInfo* pOffer = NULL;
...
bool CClientManager::RecvOfflineShopRemoveItemPacket(const char* data) //patchme
{
	...
	offlineshop::CShopCache::TShopCacheInfo* pShop = NULL;
...
bool CClientManager::RecvOfflineShopForceClose(const char* data)
{
	...
	offlineshop::CShopCache::TShopCacheInfo* pInfo=NULL;
...
bool CClientManager::RecvOfflineShopOfferAccepted(const char* data)
{
	...
	if (!offerIDs.empty())
	{
		...
		for (DWORD i = 0; i < offerIDs.size(); i++)
		{
			...
			offlineshop::COfferCache::TOfferCacheInfo* pOfferToRemove = NULL;
...
bool CClientManager::RecvOfflineShopAuctionAddOffer(const char* data)
{
	...
	offlineshop::TAuctionOfferInfo* pOffer=NULL;
...
void CClientManager::OfflineshopExpiredShop(DWORD dwID)
{
	...
	offlineshop::COfferCache::OFFERIDVEC* pVec=NULL;
	offlineshop::COfferCache::TOfferCacheInfo* pOffer=NULL;
...
void CClientManager::OfflineshopExpiredAuction(DWORD dwID)
{
	...
	offlineshop::TAuctionInfo* pAuction=NULL;
...
{
	offlineshop::CSafeboxCache::TSafeboxCacheInfo* pSafebox=NULL;
...
void CClientManager::RemoveOfferOnShopItem(DWORD dwItemID)
{
	std::vector<DWORD> offerIDs;
	m_offlineshopOfferCache.GetOffersByItemID(dwItemID, offerIDs);
