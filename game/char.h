...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	public:
		void			SyncQuickslot(BYTE bType, BYTE bOldPos, DWORD bNewPos);
		...
		void			SetMarriageRingLastUseTime();

#ifdef __ENABLE_NEW_OFFLINESHOP__
	public:
		offlineshop::CShop* GetOfflineShop()  const
		{
			return m_pkOfflineShop;
		}
		void					SetOfflineShop (offlineshop::CShop* pkShop)
		{
			m_pkOfflineShop = pkShop;
		}

		offlineshop::CShop* GetOfflineShopGuest()  const
		{
			return m_pkOfflineShopGuest;
		}
		...
		GetShopSafebox()  const
		{
			return m_pkShopSafebox;
		}
		void					SetShopSafebox (offlineshop::CShopSafebox* pk);

		void					SetAuction (offlineshop::CAuction* pk)
		{
			m_pkAuction = pk;
		}
		...
		offlineshop::CAuction* GetAuction() const
		{
			return m_pkAuction;
		}
		offlineshop::CAuction* GetAuctionGuest() const
		{
			return m_pkAuctionGuest;
		}


		//offlineshop-updated 05/08/19
		void					SetLookingOfflineshopOfferList (bool is)
		{
			m_bIsLookingOfflineshopOfferList = is;
		}
		bool					IsLookingOfflineshopOfferList() const
		{
			return m_bIsLookingOfflineshopOfferList;
		}

		// patch with warp check
		int						GetOfflineShopUseTime() const
		{
			return m_iOfflineShopUseTime;
		}
	...
	public:
		...
		long long				ComputeRefineFee(long long iCost, int iMultiply = 5) const;
...
//		bool			DropItem(TItemPos Cell, BYTE bCount=0);
		...
		bool			RefineInformation(DWORD bCell, BYTE bType, int iAdditionalCell = -1);
...
//		void			AutoGiveItem(LPITEM item, bool longOwnerShip = false);
		...
		std::pair<bool, LPITEM>	AutoStackItemProto(DWORD dwItemVnum, short & bCount);
...
//		void			OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount);
	...
	public:
		struct TBattleInfo
		{
			...
			TBattleInfo() : iTotalDamage(0), iAggro(0) {} // Varsayılan yapıcı eklendi
...
//		NIKAH_YUZUGU_COOLDOWN
	private:
		DWORD m_dwLastMarriageRingUseTime;
//		NIKAH_YUZUGU_COOLDOWN
	...
	public:
		...
		bool	IsBioFastGive() const { return m_IsFastGive; }
		void	SetBioFastGive(const bool value) {  m_IsFastGive = value; }

		int		GetFastBioTime() const { return m_FastGiveTime; }
		void	SetFastBioTime() {  m_FastGiveTime = thecore_pulse(); }
	...
	public:
		...
		TPlayerRanking	GetRanking() const { return m_points.ranking; }
		void			SetRanking(const TPlayerRanking val) { m_points.ranking = val; }

		uint64_t		GetStoneDamage() const { return m_points.ranking.ullStoneDamage; }
		void 			SetStoneDamage(const uint64_t val) { m_points.ranking.ullStoneDamage = val; }

		uint32_t		GetDestroyedStone() const { return m_points.ranking.uiDestroyedStone; }
		void 			SetDestroyedStone(const uint32_t val) { m_points.ranking.uiDestroyedStone = val; }

		uint32_t		GetDestroyedBoss() const { return m_points.ranking.uiDestroyedBoss; }
		void 			SetDestroyedBoss(const uint32_t val) { m_points.ranking.uiDestroyedBoss = val; }

		uint32_t		GetUsedSwitcher() const { return m_points.ranking.uiUsedSwitcher; }
		void 			SetUsedSwitcher(const uint32_t val) { m_points.ranking.uiUsedSwitcher = val; }

		uint32_t		GetOpenedChest() const { return m_points.ranking.uiOpenedChest; }
		void 			SetOpenedChest(const uint32_t val) { m_points.ranking.uiOpenedChest = val; }

		uint32_t		GetComplatedDungeons() const { return m_points.ranking.uiComplatedDungeon; }
		void 			SetComplatedDungeons(const uint32_t val) { m_points.ranking.uiComplatedDungeon = val; }

		uint64_t		GetBossDamage() const { return m_points.ranking.ullBossDamage; }
		void 			SetBossDamage(const uint64_t val) { m_points.ranking.ullBossDamage = val; }

		uint32_t		GetBarones() const { return m_points.ranking.uibarones; }
		void 			SetBarones(const uint32_t val) { m_points.ranking.uibarones = val; }

		uint32_t		GetEjder() const { return m_points.ranking.uiejder; }
		void 			SetEjder(const uint32_t val) { m_points.ranking.uiejder = val; }

		uint32_t		GetAzrail() const { return m_points.ranking.uiazrail; }
		void 			SetAzrail(const uint32_t val) { m_points.ranking.uiazrail = val; }

		uint32_t		GetLusifer() const { return m_points.ranking.uilusifer; }
		void 			SetLusifer(const uint32_t val) { m_points.ranking.uilusifer = val; }

		uint32_t		GetKirmizi() const { return m_points.ranking.uikirmizi; }
		void 			SetKirmizi(const uint32_t val) { m_points.ranking.uikirmizi = val; }

		uint32_t		GetRazador() const { return m_points.ranking.uirazador; }
		void 			SetRazador(const uint32_t val) { m_points.ranking.uirazador = val; }

		uint32_t		GetNemere() const { return m_points.ranking.uinemere; }
		void 			SetNemere(const uint32_t val) { m_points.ranking.uinemere = val; }

		uint64_t		GetPlayerDamage() const { return m_points.ranking.ullplayerdamage; }
		void 			SetPlayerDamage(const uint64_t val) { m_points.ranking.ullplayerdamage = val; }

		uint32_t		GetPlayerKill() const { return m_points.ranking.uiplayerkill; }
		void 			SetPlayerKill(const uint32_t val) { m_points.ranking.uiplayerkill = val; }
#endif
	...
	public:
		...
		bool		HasSameHWID(LPCHARACTER ch, LPCHARACTER ch2);
	...
	public:
		...
		void SaveCShieldData();
		void LoadCShieldData(const TCShield* tab);
#ifdef ENABLE_CAPTCHA
		bool CShieldCaptcha(DWORD errorCode);
		void ResetCShieldCaptcha();
#endif
#endif

#ifdef __ENABLE_STAGE_SYSTEM__
	public:
		void SetStageLevel(uint8_t level) { m_stage_level = level; }
		uint8_t GetStageLevel() const { return m_stage_level; }
		void SetCountdownTimer(time_t timer) { m_countdown_timer = timer; }
		time_t GetCountdownTimer() const { return m_countdown_timer; }
		void StageUpgrade(BYTE item_pos);

	protected:
		uint8_t m_stage_level;
		time_t m_countdown_timer;
#endif
};
