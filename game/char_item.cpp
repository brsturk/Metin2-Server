...
struct MapMinLevelLimit {
	int mapIndex;
	int minLevel;
};

const MapMinLevelLimit limits[] = {
	{67, 60},	// Hayalet orman
	{68, 65},	// Kızıl orman
	{104, 45},	// Orumcek zindani 1.kat
	{64, 25},	// Seungryong vadisi
	{61, 45},	// Sohan dagi
	{63, 25},	// Yongbi colu
	{62, 55},	// Doyyumhwan
	{66, 40},	// Seytan kulesi
...
bool IsLevelAllowed(int mapIndex, int level)
{
	for (int i = 0; i < sizeof(limits)/sizeof(limits[0]); ++i)
	{
		if (limits[i].mapIndex == mapIndex)
			return level >= limits[i].minLevel;
	}
	return true;
}
...
#ifdef CHAT_SUCCESS_UPGRADE_TEXT
			if (pkNewItem->GetRefineLevel() >= 7 || pkNewItem->GetRefineLevel() >= 8 || pkNewItem->GetRefineLevel() >= 9) 
			{		
...
bool CHARACTER::RefineInformation(DWORD bCell, BYTE bType, int iAdditionalCell)
{
#if defined(__SPECIAL_INVENTORY_SYSTEM__)
	...
	switch (item->GetType())
	{
		...
		case ITEM_QUEST:
			...
			if (item->GetVnum() == 70002) // perma
			{
				...
				AddAffect(AFFECT_PICKUP_ENABLE, POINT_NONE, 0, AFF_NONE, 60 * 60 * 8765, 0, 0, false); // 1 yil
				ITEM_MANAGER::instance().RemoveItem(item);
				ChatPacket(CHAT_TYPE_INFO, "[LS;2032]");
				return true;
			}

			if (item->GetVnum() == 60005) // 4s
			{
				CAffect* affect = FindAffect(AFFECT_PICKUP_ENABLE);
				if (!affect)
				{
				...
				AddAffect(AFFECT_PICKUP_ENABLE, POINT_NONE, 0, AFF_NONE, 60 * 60 * 4, 0, 0, false); // 4 saat
				ITEM_MANAGER::instance().RemoveItem(item);
				ChatPacket(CHAT_TYPE_INFO, "[LS;2033]");
				return true;
			}
			
			if (item->GetVnum() == 60006) // 1g
			{
				CAffect* affect = FindAffect(AFFECT_PICKUP_ENABLE);
				if (!affect)
				{
				...
				AddAffect(AFFECT_PICKUP_ENABLE, POINT_NONE, 0, AFF_NONE, 60 * 60 * 24, 0, 0, false); // 1 gun
				ITEM_MANAGER::instance().RemoveItem(item);
				ChatPacket(CHAT_TYPE_INFO, "[LS;2033]");
				return true;
			}
			
			if (item->GetVnum() == 60007) // 7g
			{
				CAffect* affect = FindAffect(AFFECT_PICKUP_ENABLE);
				if (!affect)
				{
				...
				AddAffect(AFFECT_PICKUP_ENABLE, POINT_NONE, 0, AFF_NONE, 60 * 60 * 168, 0, 0, false); // 7 gun
				ITEM_MANAGER::instance().RemoveItem(item);
				ChatPacket(CHAT_TYPE_INFO, "[LS;2034]");
				return true;
			}
			
			if (item->GetVnum() == 60008) // 15g
			{
				CAffect* affect = FindAffect(AFFECT_PICKUP_ENABLE);
				if (!affect)
				{
				...
				AddAffect(AFFECT_PICKUP_ENABLE, POINT_NONE, 0, AFF_NONE, 60 * 60 * 360, 0, 0, false); // 15 gun
				ITEM_MANAGER::instance().RemoveItem(item);
...
//			break;
		...
		case ITEM_CAMPFIRE:
		{
			// Kamp ate?i ozelli?i sadele?tirilmi? sistemde kaldırıldı
			ChatPacket(CHAT_TYPE_INFO, "[LS;2088]");
			return false;
		}
					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case 50263:
								{
									...
									if (item2->IsSealed())
									{
										ChatPacket(CHAT_TYPE_INFO, "[LS;2054]");
										return false;
									}
									if (item2->GetType() != ITEM_WEAPON && item2->GetType() != ITEM_ARMOR && item2->GetType() != ITEM_COSTUME)
										return false;
									item2->SetSealBind();
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("muhur_basarili"));
									{
										char buf[21];
							...
							case 50264:
								{
									...
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ayirma_basarili"));
									{
										char buf[21];
										snprintf(buf, sizeof(buf), "%u", item2->GetID());
							...
							case ITEM_MARRIAGE_RING:
							{
								...
								if (pMarriage)
								{
									DWORD dwTargetPID = pMarriage->GetOther(GetPlayerID());
									LPCHARACTER pTarget = CHARACTER_MANAGER::instance().FindByPID(dwTargetPID);

									if (!pTarget)
									{
										ChatPacket(CHAT_TYPE_INFO, "[LS;2090]");
										break;
									}

									// Arena kontrolu
									if (pMarriage->ch1 && CArenaManager::instance().IsArenaMap(pMarriage->ch1->GetMapIndex()))
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("대련 중에는 이용할 수 없는 물품입니다."));
										break;
									}
									if (pMarriage->ch2 && CArenaManager::instance().IsArenaMap(pMarriage->ch2->GetMapIndex()))
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("대련 중에는 이용할 수 없는 물품입니다."));
										break;
									}

									int targetMapIndex = pTarget->GetMapIndex();
									int myLevel = GetLevel();

									if (!IsLevelAllowed(targetMapIndex, myLevel))
									{
										ChatPacket(CHAT_TYPE_INFO, "[LS;2091]");
										break;
									}
...
//								break;
							...
							case 72357: case 72358: case 72363: // SKILL_MONSTER_BONUS
							case 72359: case 72360: case 72364: // SKILL_BOSS_BONUS
							case 72361: case 72362: case 72365: // SKILL_HP_BONUS
							case 72366: case 72367: case 72368: // SKILL_METIN_BONUS
#endif
								{
									if (IsPolymorphed() == true)
									{
									...
									switch (item->GetVnum())
									{
										...
										case 50325: case 50326:
											...
											break;
#ifdef ENABLE_NEW_PASSIVE_SKILL
										case 72357: case 72358: case 72363:
											dwSkillVnum = SKILL_MONSTER_BONUS;
											break;
										case 72359: case 72360: case 72364:
											dwSkillVnum = SKILL_BOSS_BONUS;
											break;
										case 72361: case 72362: case 72365:
											dwSkillVnum = SKILL_HP_BONUS;
											break;
										case 72366: case 72367: case 72368:
											dwSkillVnum = SKILL_METIN_BONUS;
											break;
#endif
										default:
											return false;
							...
							case 56009:
							{
								...
								auto		affect_type		= UNVAN_BONUS;
								auto		apply_type		= aApplyInfo[item->GetValue(1)].bPointType;
								auto		apply_value		= item->GetValue(2);
								ChangeTitle(0);
								auto unvan = item->GetValue(0);
								UpdateTitle(unvan);
...
#endif
#ifdef ENABLE_COSTUME_RING_SYSTEM
									 || item2->GetSubType() == COSTUME_HILAL 
									 || item2->GetSubType() == COSTUME_LOLIPOP 
									 || item2->GetSubType() == COSTUME_GUC
									 || item2->GetSubType() == COSTUME_SEVGI
									 || item2->GetSubType() == COSTUME_SEVGI2
									 || item2->GetSubType() == COSTUME_UYUM
#endif
									))
								{
									ChatPacket(CHAT_TYPE_INFO, "[LS;2048]");
							...
							case 84016:
							{
								...
								int pre_up_time = item2->GetSocket(2);
								int max_up_time = item2->GetValue(2);
								/* macros */
								if ((existing_duration == 0) || (new_duration - time(0) >= limit_time) || (pre_up_time >= max_up_time))
								{
								...
								item2->SetSocket(2, pre_up_time + 1);
							}
							break;
#endif
					...
					case USE_ABILITY_UP:
						{
							switch (item->GetValue(0))
							{
								case APPLY_MOV_SPEED:
									...
									if (item->GetVnum() == 27866)
									{
										if (FindAffect(SAZAN_BLEND_POTION))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("이미 효과가 걸려 있습니다."));
											return false;
										}
										AddAffect(SAZAN_BLEND_POTION, POINT_MOV_SPEED, item->GetValue(2), 0, item->GetValue(1), 0, true);
									}
									else
									{
								...
								case APPLY_ATT_SPEED:
									...
									if (item->GetVnum() == 27868)
									{
										if (FindAffect(OTSAZAN_BLEND_POTION))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("이미 효과가 걸려 있습니다."));
											return false;
										}
										AddAffect(OTSAZAN_BLEND_POTION, POINT_ATT_SPEED, item->GetValue(2), 0, item->GetValue(1), 0, true);
									}
									else
									{
								...
								case APPLY_STR:
									if (item->GetVnum() == 27870)
									{
										if (FindAffect(ZARGANA_BLEND_POTION))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("이미 효과가 걸려 있습니다."));
											return false;
										}
										AddAffect(ZARGANA_BLEND_POTION, POINT_ST, item->GetValue(2), 0, item->GetValue(1), 0, true);
									}
									else
									{
								...
								case APPLY_DEX:
									if (item->GetVnum() == 27873)
									{
										if (FindAffect(HAMSI_BLEND_POTION))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("이미 효과가 걸려 있습니다."));
											return false;
										}
										AddAffect(HAMSI_BLEND_POTION, POINT_DX, item->GetValue(2), 0, item->GetValue(1), 0, true);
									}
									else
									{
								...
								case APPLY_INT:
									if (item->GetVnum() == 27876)
									{
										if (FindAffect(YAYIN_BLEND_POTION))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("이미 효과가 걸려 있습니다."));
											return false;
										}
										AddAffect(YAYIN_BLEND_POTION, POINT_IQ, item->GetValue(2), 0, item->GetValue(1), 0, true);
									}
									else
									{
								...
								case APPLY_MALL_EXPBONUS:
									if (item->GetVnum() == 27883)
									{
										if (FindAffect(SUDAK_BLEND_POTION))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("이미 효과가 걸려 있습니다."));
											return false;
										}
										AddAffect(SUDAK_BLEND_POTION, POINT_MALL_EXPBONUS, item->GetValue(2), 0, item->GetValue(1), 0, true);
									}
									break;
					...
					case USE_ADD_ATTRIBUTE2:
						{
							...
							switch (item->GetSubType())
							{
								...
								case USE_CHANGE_ATTRIBUTE2 : // @fixme123
									...
									if(item2->IsSealed())
									{
										ChatPacket(CHAT_TYPE_INFO, "[LS;2057]");
										return false;
									}
#endif
		...
		case ITEM_BLEND:
			...
			if (Blend_Item_find(item->GetVnum()))
			{
				...
				switch (apply_type)
				{
					...
					case POINT_ATTBONUS_MONSTER: affect_type = CANAVAR_BLEND_POTION; break;
					case POINT_ATTBONUS_METIN: affect_type  = METIN_BLEND_POTION; break;
					case POINT_ATTBONUS_BOSS: affect_type = PATRON_BLEND_POTION; break;
					case POINT_ATTBONUS_HUMAN: affect_type = PEMBE_BLEND_POTION; break;
				}
#endif
				if (FindAffect(affect_type, apply_type))
				{
...
bool CHARACTER::DropItem(TItemPos Cell, short bCount)
{
	...
	if (item->IsSealed())
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;2058]");
		return false;
	}
#endif
				...
				if ((item->IsSkillBook() || item->IsUpgradeItem() || item->IsStone() || item->IsGiftBox()) && item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
					...
					for (int i = SKILL_BOOK_INVENTORY_SLOT_START; i < GIFT_BOX_INVENTORY_SLOT_END; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							auto bCount2 = MIN(ITEM_MAX_COUNT - item2->GetCount(), bCount);
							bCount -= bCount2;
							...
							if (bCount == 0)
							{
								ChatPacket(CHAT_TYPE_INFO, "[LS;220;%s]", item2->GetName());
								M2_DESTROY_ITEM(item);
	...
	if (p->dwFlags & ITEM_FLAG_STACKABLE && p->bType != ITEM_BLEND)
	{
		for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
		{
			...
			if (item->GetVnum() == dwItemVnum && FN_check_item_socket(item))
			{
				...
				auto bCount2 = MIN(ITEM_MAX_COUNT  - item->GetCount(), bCount);
				bCount -= bCount2;
	...
	if (p->dwFlags & ITEM_FLAG_STACKABLE/* && p->bType != ITEM_BLEND*/)
	{
		for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
		{
			...
			if (item->GetVnum() == dwItemVnum && FN_check_item_socket(item))
			{
				...
				{
					if (bMsg)
						ChatPacket(CHAT_TYPE_INFO, "[LS;220;%s]", item->GetName());
...
bool CHARACTER::CanReceiveItem(LPCHARACTER from, LPITEM item) const
{
	...
	if(item->IsSealed())
	{
		from->ChatPacket(CHAT_TYPE_INFO, "[LS;2059]");
		return false;
	}
#endif
...
#ifdef ENABLE_SKILL_WITH_POLYMORPH_FIX
				if (IsAffectFlag(AFF_GEOMGYEONG))
					RemoveAffect(SKILL_GEOMKYUNG);
				if (IsAffectFlag(AFF_GWIGUM))
					RemoveAffect(SKILL_GWIGEOM);
...
#endif
#ifdef ENABLE_COSTUME_RING_SYSTEM
	LPITEM HilalCostume = GetWear(WEAR_COSTUME_HILAL);
	LPITEM LolipopCostume = GetWear(WEAR_COSTUME_LOLIPOP);
	LPITEM GucCostume = GetWear(WEAR_COSTUME_GUC);
	LPITEM SevgiCostume = GetWear(WEAR_COSTUME_SEVGI);
	LPITEM Sevgi2Costume = GetWear(WEAR_COSTUME_SEVGI2);
	LPITEM UyumCostume = GetWear(WEAR_COSTUME_UYUM);
#endif
...
#ifdef ENABLE_COSTUME_RING_SYSTEM
	if (HilalCostume)
		return UnequipItem(HilalCostume);
	if (LolipopCostume)
		return UnequipItem(LolipopCostume);
	if (GucCostume)
		return UnequipItem(GucCostume);
	if (SevgiCostume)
		return UnequipItem(SevgiCostume);
	if (Sevgi2Costume)
		return UnequipItem(Sevgi2Costume);
	if (UyumCostume)
		return UnequipItem(UyumCostume);
#endif
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
	...
	ITEM_MANAGER::instance().RemoveItem(item);
	ChatPacket(CHAT_TYPE_INFO, "[LS;2028]", item->GetName());
...
bool CHARACTER::SellItem(TItemPos Cell, BYTE bCount)
{
	...
	if(item->IsSealed())
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;2062]");
		return true;
	}
#endif
	...
	ChatPacket(CHAT_TYPE_INFO, "[LS;2027]", item->GetName());
					...
					if (wCount == 0)
					{
						if (printMsg)
						{
							if (realCount > 1)
								...
								ChatPacket(CHAT_TYPE_INFO, "[LS;2038;%s;%d]", item->GetName(), realCount);
							else
