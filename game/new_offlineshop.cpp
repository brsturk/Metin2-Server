...
namespace offlineshop
{
	...
	*/
	...
	CShopItem::CShopItem(LPITEM pItem, const TPriceInfo& sPrice, BYTE bWindowType, DWORD dwID): m_dwID(0)
	{
		...
		if (pItem)
		{
			...
			m_itemInfo.iEvolution = pItem->GetEvolution();
#endif
		}
...
#endif
#ifdef ENABLE_WEAPON_EVOLUTION_SYSTEM
		item->SetEvolution(m_itemInfo.iEvolution);
#endif
	...
	-	(private) refresh list item to a guest or all (args != nullptr -> send to one, otherwise send to all)
	...
	*/
	...
	void CShop::SendNotificationClientPacket(DWORD dwItemID, long long dwItemPrice, WORD dwItemCount)
	{
		...
		pkOwner->GetDesc()->Packet(&subpack, sizeof(subpack));
	}
#endif
