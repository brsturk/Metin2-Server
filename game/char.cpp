...
void CHARACTER::Initialize()
{
	...
	m_pkBiologNtfEvent = NULL;
#endif
//	NIKAH_YUZUGU_COOLDOWN
	m_dwLastMarriageRingUseTime = 0;
//	NIKAH_YUZUGU_COOLDOWN
	// MINING
...
#endif
#ifdef __ENABLE_STAGE_SYSTEM__
	tab.stage_level = GetStageLevel();
	tab.countdown_timer = GetCountdownTimer();
#endif
...
void CHARACTER::SetPlayerProto(const TPlayerTable * t)
{
	...
	SetGold(t->gold);
#ifdef __ENABLE_STAGE_SYSTEM__
	SetStageLevel(t->stage_level);
	SetCountdownTimer(t->countdown_timer);
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
	SetCheque(t->cheque);
#endif
...
void CHARACTER::ComputePoints()
{
	...
	if (IsPC())
	{
...
#ifdef ENABLE_NEW_PASSIVE_SKILL
		{
			pkSk = CSkillManager::instance().Get(SKILL_MONSTER_BONUS); // MONSTER
		...
		{
			pkSk = CSkillManager::instance().Get(SKILL_BOSS_BONUS); // BOSS
...

		{
			...
			if (NULL != pkSk)
			{
				...
				ekle = static_cast<int>(pkSk->kPointPoly.Eval());
				PointChange(POINT_MAX_HP_PCT,ekle);
			}
		}

		{
			pkSk = CSkillManager::instance().Get(SKILL_METIN_BONUS); // METIN

			if (NULL != pkSk)
			{
				pkSk->SetPointVar("k", 1.0f * GetSkillPower(SKILL_METIN_BONUS) / 100.0f);

				ekle = static_cast<int>(pkSk->kPointPoly.Eval());
				PointChange(POINT_ATTBONUS_METIN,ekle);
			}
		}
#endif
...
//		{
//#ifndef REDUCED_LOGS
//			LogManager::instance().CShieldLog(this, 10001, false);
//#endif
...
bool CHARACTER::Sync(long x, long y)
{
	LPSECTREE current_tree = GetSectree();
	if (!current_tree)
	{
		sys_err("<CHARACTER::Sync> Sectree is NULL! (Name: %s, Map: %d)", GetName(), GetMapIndex());
		return false;
	}
	...
	if (!new_tree)
	{
		sys_err("No Tree: (x: %ld, y: %ld, Name: %s, Map: %d)", x, y, GetName(), GetMapIndex());
		x = GetX();
		y = GetY();
	...
	if (IsPC())
	{
		if (GetCShield()->CheckMoveSpeedhack(x, y, get_dword_time(), IsRiding(), GetMoveSpeed()))
		{
			...
			Show(GetMapIndex(), GetX(), GetY(), 0);
			Stop();
			return false; // Hile tespit edilirse senkronizasyonu durdur
		}
	}
#endif
...
//					snprintf(levelduyuru, sizeof(levelduyuru), "<Bilgi> [CH%d]: [%s] isimli oyuncumuz [%u] seviye olmustur tebrikler !", g_bChannel, GetName(), GetLevel()); 
...
//			}
			...
			if (val == gPlayerMaxLevel)
			{
				char levelduyuru[QUERY_MAX_LEN];
				snprintf(levelduyuru, sizeof(levelduyuru), "<Bilgi> [CH%d]: [%s] isimli oyuncumuz [%u] seviye olmustur tebrikler !", 
						 g_bChannel, GetName(), GetLevel());
				BroadcastNotice(levelduyuru);
			}
#endif
...
#ifdef ENABLE_MAP_FISH_FIX
	if (GetMapIndex() == 41 || GetMapIndex() == 21 || GetMapIndex() == 1 ||	// Mavi - Sari - Kirmizi 1.Koy
		GetMapIndex() == 43 || GetMapIndex() == 23 || GetMapIndex() == 3 ||	// Mavi - Sari - Kirmizi 2.Koy
		GetMapIndex() == 63 || GetMapIndex() == 64)							// Yongbi Colu - Seungryong Vadisi
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;2086]");
		return;
	}
#endif
...
#ifdef ENABLE_CSHIELD
	if (battle_is_attackable(pkChrCauser, this))
	{
		if (IsMonster() && GetMobRank() >= MOB_RANK_BOSS)
			pkChrCauser->GetCShield()->SetBossCount(0);
		else if (IsStone())
			pkChrCauser->GetCShield()->SetMetinCount(0);
	}
#endif
	DWORD vid = GetVID();
...
#ifdef __ENABLE_NEW_OFFLINESHOP__
							|| GetOfflineShopGuest() || GetShopSafebox()
#endif
					)
					{
...
#ifdef __ENABLE_NEW_OFFLINESHOP__
							|| pkChrCauser->GetOfflineShopGuest() || pkChrCauser->GetShopSafebox()
#endif
					)
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("다른 거래중(창고,교환,상점)에는 개인상점을 사용할 수 없습니다."));
...
void CHARACTER::DetermineDropMetinStone()
{
	...
	{
		// NOTE : Zihin tasi kesesi eklendi.
	};
...
bool CHARACTER::WarpToPID(DWORD dwPID)
{
	LPCHARACTER victim = CHARACTER_MANAGER::instance().FindByPID(dwPID);
	if (victim)
	{
		int mapIdx = victim->GetMapIndex();
		if (IS_SUMMONABLE_ZONE(mapIdx) && CAN_ENTER_ZONE(this, mapIdx))
		{
			WarpSet(victim->GetX(), victim->GetY());
			return true;
		}
	...
	else
	{
		...
		if (!IS_SUMMONABLE_ZONE(pcci->lMapIndex) || !CAN_ENTER_ZONE(this, pcci->lMapIndex))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("상대방이 있는 곳으로 워프할 수 없습니다."));
			return false;
		}

		// Bu durumda henuz ı?ınlama yapılmadı?ı icin cooldown BA?LATILMAMALI
		TPacketGGFindPosition p;
		p.header = HEADER_GG_FIND_POSITION;
		p.dwFromPID = GetPlayerID();
		p.dwTargetPID = dwPID;
...
long long CHARACTER::ComputeRefineFee(long long iCost, int iMultiply) const
{
...
void CHARACTER::PayRefineFee(long long iTotalMoney)
{
	long long iFee = iTotalMoney / 10;
	CGuild* pGuild = GetRefineGuild();

	long long iRemain = iTotalMoney;
...
bool CHARACTER::CanWarp() const
{
	...
	if (GetOfflineShopGuest() || GetShopSafebox() || GetAuctionGuest())
	{
		return false;
	}
#endif
...
bool CHARACTER::WonExchange(const std::string & option, int value)
{
	...
	if (option == "sell")
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You successfully sold %d won for %lld yangs."), value, totalYang);
	...
	else if (option == "buy")
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You don't have enough yangs for the exchange. Required %lld, owned %d."), totalYang, GetGold());
			return false;
		}
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You successfully bought %d won for %lld yangs."), value, totalYang);
		PointChange(POINT_GOLD, -totalYang, true);
		PointChange(POINT_CHEQUE, value, true);
	}
	else
...
bool CHARACTER::ChangeChannel(BYTE newChannel)
{
	...
	if (!g_stProxyIP.empty())
		p.lAddr = inet_addr(g_stProxyIP.c_str());
#endif
#if defined(ENABLE_ANTI_MULTIPLE_FARM)
	UpdateCharacterWarpCheck(true);
#endif
	p.wPort	= newPort;
	GetDesc()->Packet(p);
...
bool CHARACTER::HasSameHWID(LPCHARACTER ch, LPCHARACTER ch2)
{

	const LPDESC d1 = ch ? ch->GetDesc() : nullptr;
	const LPDESC d2 = ch2 ? ch2->GetDesc() : nullptr;
	...
	return (strcmp(d1->GetLoginMacAdress(), d2->GetLoginMacAdress()) == 0);
}
...
void CHARACTER::SortInventoryItems()
{
	...
	if (lastSortInventoryPulse > iPulse)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, "[LS;1953;%d]", seconds);
		return;
	}
	...
	std::vector<LPITEM>::iterator it = v.begin();
	while (it != v.end())
	{
		LPITEM item = *(it++);
		...
		if (item)
		{
			...
			if (item->GetType() == ITEM_SKILLBOOK || item->GetType() == ITEM_SKILLFORGET || item->GetType() == ITEM_USE)
			{
				{
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						LPITEM book_item = GetInventoryItem(i);

						if (book_item && book_item->GetVnum() == item->GetVnum() && (item->GetCount() + book_item->GetCount()) <= ITEM_MAX_COUNT)
						{
							bool bStopSockets = false;

							for (int k = 0; k < ITEM_SOCKET_MAX_NUM; ++k)
							{
								if (book_item->GetSocket(k) != item->GetSocket(k))
								{
									bStopSockets = true;
									break;
								}
							...
							int bCount = MIN(ITEM_MAX_COUNT - book_item->GetCount(), item->GetCount());
							book_item->SetCount(book_item->GetCount() + bCount);
			...
			else
			{
				...
				if (p && p->dwFlags & ITEM_FLAG_STACKABLE && p->bType != ITEM_BLEND && p->bSubType != USE_TALISMAN &&
...
void CHARACTER::SortSpecialInventoryItems(BYTE type)
{
	...
	if (lastSortSpecialInventoryPulse > iPulse)
	{
	...
	std::vector<LPITEM>::iterator it = vecItems.begin();
	...
	SetSortSpecialInventoryPulse(thecore_pulse() + PASSES_PER_SEC(10));
	ChatPacket(CHAT_TYPE_INFO, "[LS;1951]");
}
#endif
#endif
...
EVENTFUNC(cshield_event)
{
	...
	if (cShield > 1)
	{
		...
		if (cShield == 2)
			LogManager::instance().CShieldLog(ch, 10002);
#ifndef REDUCED_LOGS
		else
			LogManager::instance().CShieldLog(ch, 10003);
#else
		else
		{
			sys_log(0, "CSHIELD: Player async %s", ch->GetName());
...
void CHARACTER::LoadCShieldData(const TCShield* tab)
{
	GetCShield()->SetMetinCount(tab->metinCount);
	GetCShield()->SetBossCount(tab->bossCount);
#ifdef ENABLE_CAPTCHA
	GetCShield()->SetCaptchaTime(tab->captchaTime);
	GetCShield()->SetCaptchaCount(tab->captchaCount);
#endif
}

void CHARACTER::SaveCShieldData()
{
	TCShield tab = { 0 };

	tab.playerId = GetPlayerID();
	strlcpy(tab.name, GetName(), sizeof(tab.name));
	tab.metinCount = GetCShield()->GetMetinCount();
	tab.bossCount = GetCShield()->GetBossCount();
#ifdef ENABLE_CAPTCHA
	tab.captchaTime = GetCShield()->GetCaptchaTime();
	tab.captchaCount = GetCShield()->GetCaptchaCount();
#endif

	db_clientdesc->DBPacketHeader(HEADER_GD_CSHIELD_SAVE, 0, sizeof(TCShield));
	db_clientdesc->Packet(&tab, sizeof(TCShield));
}
#endif

#ifdef ENABLE_CAPTCHA
void CHARACTER::ResetCShieldCaptcha()
{
	GetCShield()->SetCaptchaTime(time(0));
}

bool CHARACTER::CShieldCaptcha(DWORD errorCode)
{
	if (GetDungeon())
		return false;

	if (time(0) > GetCShield()->GetCaptchaTime())
	{
#ifdef ENABLE_CAPTCHA_AND_DEV_WHITELIST
		if (GetDesc() && IsAccountWhitelisted(GetDesc()->GetAccountTable().login))
		{
			GetCShield()->SetCaptchaTime(time(0) + (ENABLE_CAPTCHA * 10 * 60));
			return false;
		}
#endif

		if (GetCShield()->GetCaptchaAfterLoginTime() == 0)
			GetCShield()->SetCaptchaAfterLoginTime(get_dword_time() + (1 * 60 * 1000));

		if (thecore_heart->pulse - (int)GetCShield()->GetLastCaptchaPulse() > passes_per_sec * 25)
		{
			if (errorCode == 10007) // delay only for pve
				GetCShield()->SetLastCaptchaPulse(thecore_heart->pulse);
			GetCShield()->SetCaptchaResult(number(100, 999));

			TPacketGCCaptcha packet;
			packet.header = HEADER_GC_CAPTCHA;
			strlcpy(packet.captchaImage, GetCShield()->GenerateCaptcha(GetCShield()->GetCaptchaResult()).c_str(), sizeof(packet.captchaImage));

			LPDESC d = GetDesc();
			if (d)
				GetDesc()->Packet(&packet, sizeof(TPacketGCCaptcha));

			if (GetCShield()->GetCaptchaCount() < 3)
				GetCShield()->SetCaptchaCount(GetCShield()->GetCaptchaCount() + 1);

			if (errorCode == 10007) // delay only for pve
			{
				if (GetCShield()->GetCaptchaCount() >= 3)
					GetCShield()->SetCaptchaSessionCount(GetCShield()->GetCaptchaSessionCount() + 1);
			}
			else
				GetCShield()->SetCaptchaSessionCount(GetCShield()->GetCaptchaSessionCount() + 1);
		}

		if (GetCShield()->GetCaptchaCount() == 3 && GetCShield()->GetCaptchaSessionCount() >= 2)
			LogManager::instance().CShieldLog(this, errorCode);

		if (get_dword_time() > GetCShield()->GetCaptchaAfterLoginTime() || GetCShield()->GetCaptchaCount() >= 3)
			return true;
	}
...
void CHARACTER::SetMarriageRingLastUseTime()
{
	m_dwLastMarriageRingUseTime = get_dword_time();
}
