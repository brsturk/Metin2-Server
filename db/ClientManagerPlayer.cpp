...
{
	size_t queryLen = snprintf(pszQuery, querySize,
			"UPDATE player%s SET "
			"job = %d, "
			"voice = %d, "
			"dir = %d, "
			...
			"new_ranking_stone_damage = %llu, "
			"new_ranking_destroyed_stone = %u, "
			"new_ranking_destroyed_boss = %u, "
			"new_ranking_used_switcher = %u, "
			"new_ranking_opened_chest = %u, "
			"new_ranking_complated_dungeons = %u, "
			"new_ranking_boss_damage = %llu, "
			"bossbarones = %u, "
			"bossejder = %u, "
			"bossazrail = %u, "
			"bosslusifer = %u, "
...
#endif
#ifdef ENABLE_NEW_RANKING_SYSTEM
			pkTab->ranking.ullStoneDamage,
			pkTab->ranking.uiDestroyedStone,
			pkTab->ranking.uiDestroyedBoss,
			pkTab->ranking.uiUsedSwitcher,
			pkTab->ranking.uiOpenedChest,
			pkTab->ranking.uiComplatedDungeon,
			pkTab->ranking.ullBossDamage,
			pkTab->ranking.uibarones,
			pkTab->ranking.uiejder,
			pkTab->ranking.uiazrail,
			pkTab->ranking.uilusifer,
			pkTab->ranking.uikirmizi,
			pkTab->ranking.uirazador,
			pkTab->ranking.uinemere,
			pkTab->ranking.ullplayerdamage,
			pkTab->ranking.uiplayerkill,
#endif
#ifdef __ENABLE_STAGE_SYSTEM__
			pkTab->stage_level,
			pkTab->countdown_timer
#endif
	);
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		{
			snprintf(szQuery, sizeof(szQuery),
					"SELECT id,`window`+0,pos,count,vnum,socket0,socket1,socket2,attrtype0,attrvalue0,attrtype1,attrvalue1,attrtype2,attrvalue2,attrtype3,attrvalue3,attrtype4,attrvalue4,attrtype5,attrvalue5,attrtype6,attrvalue6,sealbind "
...
#endif
#ifdef ENABLE_NEW_RANKING_SYSTEM
				",new_ranking_stone_damage"
				",new_ranking_destroyed_stone"
				",new_ranking_destroyed_boss"
				",new_ranking_used_switcher"
				",new_ranking_opened_chest"
				",new_ranking_complated_dungeons"
				",new_ranking_boss_damage"
				",bossbarones"
				",bossejder"
				",bossazrail"
				",bosslusifer"
...
#endif
#ifdef ENABLE_NEW_BIOLOG
	str_to_number(pkTab->bioLevel, row[col++]);
	str_to_number(pkTab->bioGiven, row[col++]);
	str_to_number(pkTab->bioState, row[col++]);
	str_to_number(pkTab->bioTime, row[col++]);
	str_to_number(pkTab->bioNtf, row[col++]);
#endif
#ifdef ENABLE_NEW_RANKING_SYSTEM
	str_to_number(pkTab->ranking.ullStoneDamage, row[col++]);
	str_to_number(pkTab->ranking.uiDestroyedStone, row[col++]);
	str_to_number(pkTab->ranking.uiDestroyedBoss, row[col++]);
	str_to_number(pkTab->ranking.uiUsedSwitcher, row[col++]);
	str_to_number(pkTab->ranking.uiOpenedChest, row[col++]);
	str_to_number(pkTab->ranking.uiComplatedDungeon, row[col++]);
	str_to_number(pkTab->ranking.ullBossDamage, row[col++]);
	str_to_number(pkTab->ranking.uibarones, row[col++]);
	str_to_number(pkTab->ranking.uiejder, row[col++]);
	str_to_number(pkTab->ranking.uiazrail, row[col++]);
	str_to_number(pkTab->ranking.uilusifer, row[col++]);
	str_to_number(pkTab->ranking.uikirmizi, row[col++]);
	str_to_number(pkTab->ranking.uirazador, row[col++]);
	str_to_number(pkTab->ranking.uinemere, row[col++]);
	str_to_number(pkTab->ranking.ullplayerdamage, row[col++]);
	str_to_number(pkTab->ranking.uiplayerkill, row[col++]);
#endif
#ifdef __ENABLE_STAGE_SYSTEM__
	str_to_number(pkTab->stage_level, row[col++]);
	str_to_number(pkTab->countdown_timer, row[col++]);
#endif
...
void CClientManager::RESULT_COMPOSITE_PLAYER(CPeer * peer, SQLMsg * pMsg, DWORD dwQID)
{
	...
	switch (dwQID)
	{
		...
		case QID_CSHIELD:
			sys_log(0, "QID_CSHIELD %u %u", info->dwHandle, info->player_id);
			RESULT_CSHIELD_LOAD(peer, pSQLResult, info->dwHandle);
			break;
#endif
...
#ifdef ENABLE_NEW_RANKING_SYSTEM
const static std::vector<std::string> rowNames = {
	"level", "playtime", "new_ranking_stone_damage", "new_ranking_destroyed_stone", "new_ranking_destroyed_boss", "new_ranking_used_switcher", 
	"new_ranking_opened_chest", "new_ranking_complated_dungeons", "new_ranking_boss_damage", "bossbarones", "bossejder",
	"bossazrail", "bosslusifer", "bosskirmiziejder", "bossrazador", "bossnemere", "playerdamage", "playerkill"
};

void CClientManager::UpdateRankingInfo()
{
	m_kNewRankingInfo = {}; // clear
	char szQuery[QUERY_MAX_LEN] = {};
	for (size_t i = 0; i < rowNames.size(); ++i)
	{
		const std::string& name = rowNames.at(i);
		if (name.empty()) continue;
		snprintf(szQuery, sizeof(szQuery), "SELECT plr.name, plr.`%s`, idx.empire FROM player AS plr LEFT JOIN player_index AS idx ON plr.account_id = idx.id WHERE plr.name NOT LIKE '[%%%%]%%%%' AND plr.name NOT LIKE '[%%]' ORDER BY plr.`%s` DESC, playtime DESC LIMIT 10", name.c_str(), name.c_str());
		...
		while (NULL != (mRow = mysql_fetch_row(pMsg->Get()->pSQLResult)))
		{
			switch(i)
			{
				case 0:
				{
					strlcpy(m_kNewRankingInfo.aRankingLevelInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingLevelInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingLevelInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingLevelInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 1:
				{
					strlcpy(m_kNewRankingInfo.aRankingMinutesInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingMinutesInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingMinutesInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingMinutesInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 2:
				{
					strlcpy(m_kNewRankingInfo.aRankingStoneDamageInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingStoneDamageInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingStoneDamageInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingStoneDamageInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 3:
				{
					strlcpy(m_kNewRankingInfo.aRankingStoneDestroyInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingStoneDestroyInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingStoneDestroyInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingStoneDestroyInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 4:
				{
					strlcpy(m_kNewRankingInfo.aRankingBossDestroyInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingBossDestroyInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingBossDestroyInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingBossDestroyInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 5:
				{
					strlcpy(m_kNewRankingInfo.aRankingSwitcherInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingSwitcherInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingSwitcherInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingSwitcherInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 6:
				{
					strlcpy(m_kNewRankingInfo.aRankingOpenChestInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingOpenChestInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingOpenChestInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingOpenChestInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				case 7:
				{
					strlcpy(m_kNewRankingInfo.aRankingCompletedDungeonInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingCompletedDungeonInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingCompletedDungeonInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingCompletedDungeonInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 8:
				{
					strlcpy(m_kNewRankingInfo.aRankingBossDamageInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingBossDamageInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingBossDamageInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingBossDamageInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 9:
				{
					strlcpy(m_kNewRankingInfo.aRankingBaronesInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingBaronesInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingBaronesInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingBaronesInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 10:
				{
					strlcpy(m_kNewRankingInfo.aRankingEjderInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingEjderInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingEjderInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingEjderInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 11:
				{
					strlcpy(m_kNewRankingInfo.aRankingAzrailInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingAzrailInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingAzrailInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingAzrailInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 12:
				{
					strlcpy(m_kNewRankingInfo.aRankingLusiferInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingLusiferInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingLusiferInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingLusiferInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 13:
				{
					strlcpy(m_kNewRankingInfo.aRankingKirmiziInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingKirmiziInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingKirmiziInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingKirmiziInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 14:
				{
					strlcpy(m_kNewRankingInfo.aRankingRazadorInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingRazadorInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingRazadorInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingRazadorInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 15:
				{
					strlcpy(m_kNewRankingInfo.aRankingNemereInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingNemereInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingNemereInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingNemereInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 16:
				{
					strlcpy(m_kNewRankingInfo.aRankingPlayerDamageInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingPlayerDamageInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingPlayerDamageInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingPlayerDamageInfo[index].aRankingInfo.empire, mRow[2]);
				}
				case 17:
				{
					strlcpy(m_kNewRankingInfo.aRankingPlayerKillInfo[index].aRankingInfo.name, mRow[0], sizeof(m_kNewRankingInfo.aRankingPlayerKillInfo[index].aRankingInfo.name));
					str_to_number(m_kNewRankingInfo.aRankingPlayerKillInfo[index].aRankingInfo.value, mRow[1]);
					str_to_number(m_kNewRankingInfo.aRankingPlayerKillInfo[index].aRankingInfo.empire, mRow[2]);
				}
				break;
				default: break;
			}
	...
	for (auto& [key, vec] : m_PlayerCacheInfos) { // reset update status
			for (auto& elm : vec) {
				elm.isUpdate = false;
			}
	}
	...
	// log info
#ifdef ENABLE_NEW_RANKING_SYSTEM_LOG
	for (const auto& elm : m_kNewRankingInfo.aRankingLevelInfo)
	{
		sys_log(0, "LevelRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingMinutesInfo)
	{
		sys_log(0, "MinutesRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingStoneDamageInfo)
	{
		sys_log(0, "StoneDamageRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingStoneDestroyInfo)
	{
		sys_log(0, "StoneDestroyRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingBossDestroyInfo)
	{
		sys_log(0, "BossDestroyRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingSwitcherInfo)
	{
		sys_log(0, "SwitcherRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingOpenChestInfo)
	{
		sys_log(0, "OpenChestRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingCompletedDungeonInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
		sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingBossDamageInfo)
	{
		sys_log(0, "StoneDamageRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingBaronesInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingEjderInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingAzrailInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingLusiferInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingKirmiziInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingRazadorInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingNemereInfo)
	{
		sys_log(0, "CompletedDungeonRankingInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingPlayerDamageInfo)
	{
		sys_log(0, "aRankingPlayerDamageInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
	sys_log(0, "--------------------------------------------------");
	for (const auto& elm : m_kNewRankingInfo.aRankingPlayerKillInfo)
	{
		sys_log(0, "aRankingPlayerKillInfo name[%s] empire[%d] value[%lld]", elm.aRankingInfo.name, elm.aRankingInfo.empire, elm.aRankingInfo.value);
	}
#endif
	
}

void CClientManager::RecvNewRankingGDInfo(CPeer *peer, DWORD dwHandle, const char *c_pData)
{
	auto* pPacket = (const TNewRankingGDPacket*)c_pData;
	c_pData += sizeof(TNewRankingGDPacket);

	switch (pPacket->bSubHeader)
	{
		case NEWRANKING_SUBHEADER_GD_LOAD: { SendNewRankingGDInfoQuery(peer, dwHandle, (TNewRankingGDInfoPacket *)c_pData); } break;
		// case NEWRANKING_SUBHEADER_GD_RESET: { auto* data = (TNewRankingGDResetPacket*)c_pData; ResetRank(data->bIsWeekly); } break;
		// case NEWRANKING_SUBHEADER_GD_REWARD: { SendNewRankingRewardResultQuery(peer, dwHandle, (TNewRankingGDRewardPacket *)c_pData); } break;
		default: break;
	}
}

void CClientManager::SendNewRankingGDInfoQuery(CPeer* peer, DWORD dwHandle, TNewRankingGDInfoPacket* packet)
{
	TNewRankingDGPacket mainPacket{};
	mainPacket.bSubHeader = NEWRANKING_SUBHEADER_DG_LOAD;
	
	const auto& GetPlayerRank = [&](const std::string& name) -> uint32_t
	{
		char szQuery[QUERY_MAX_LEN] = {};
		snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*)+1 FROM player WHERE name NOT LIKE '[%%%%]%%%%' and (%s, playtime) > (SELECT %s, playtime FROM player WHERE id = %d) ORDER BY %s DESC, playtime DESC;", name.c_str(), name.c_str(), packet->dwPID, name.c_str());
		std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery));
		MYSQL_ROW mRow;

		while (NULL != (mRow = mysql_fetch_row(pMsg->Get()->pSQLResult))) {
			uint32_t rank = 0;
			str_to_number(rank, mRow[0]);
			return rank;
		}
	...
	const auto& GetPlayerValue = [&](const std::string& name) -> uint64_t
	{
		char szQuery[QUERY_MAX_LEN] = {};
		snprintf(szQuery, sizeof(szQuery), "SELECT %s FROM player WHERE id = %d", name.c_str(), packet->dwPID);
		std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery));
		MYSQL_ROW mRow;

		while (NULL != (mRow = mysql_fetch_row(pMsg->Get()->pSQLResult))) {
			uint64_t value = 0;
			str_to_number(value, mRow[0]);
			return value;
		}
	...
	TNewRankingDGRankingInfo sendPacket{};

	switch (packet->bCategory)
	{
		case 0: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingLevelInfo; break;
		case 1: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingMinutesInfo; break;
		case 2: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingStoneDamageInfo; break;
		case 3: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingStoneDestroyInfo; break;
		case 4: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingBossDestroyInfo; break;
		case 5: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingSwitcherInfo; break;
		case 6: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingOpenChestInfo; break;
		case 7: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingCompletedDungeonInfo; break;
		case 8: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingBossDamageInfo; break;
		case 9: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingBaronesInfo; break;
		case 10: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingEjderInfo; break;
		case 11: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingAzrailInfo; break;
		case 12: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingLusiferInfo; break;
		case 13: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingKirmiziInfo; break;
		case 14: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingRazadorInfo; break;
		case 15: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingNemereInfo; break;
		case 16: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingPlayerDamageInfo; break;
		case 17: sendPacket.aRankingInfo = m_kNewRankingInfo.aRankingPlayerKillInfo; break;
		default: break;
	}


	const auto& writeInfos = [&] (const uint32_t rank, const uint64_t value) {
		sendPacket.aRankingPlayerInfo.rank = rank;
		sendPacket.aRankingPlayerInfo.value = value;
	};

	const auto& it = m_PlayerCacheInfos.find(packet->dwPID);
	if (it != m_PlayerCacheInfos.end()) {
		auto& vecInfo = it->second[packet->bCategory];
		if (vecInfo.isUpdate) {
			writeInfos(vecInfo.aRankingPlayerInfo.rank, vecInfo.aRankingPlayerInfo.value);
		}
		else {
			const std::string& name = rowNames[packet->bCategory];
			const auto rank = GetPlayerRank(name);
			const auto val = GetPlayerValue(name);
			writeInfos(rank, val);

			TNewRankingPlayerCachedinfo info{};
			info.aRankingPlayerInfo.rank = rank;
			info.aRankingPlayerInfo.value = val;
			info.isUpdate = true;
			vecInfo = info;
		}
	}
		...
		const std::string& name = rowNames[packet->bCategory];
		TNewRankingPlayerCachedinfo info{};
		info.aRankingPlayerInfo.rank = GetPlayerRank(name);
		info.aRankingPlayerInfo.value = GetPlayerValue(name);
		info.isUpdate = true;
		std::vector<TNewRankingPlayerCachedinfo> vec(rowNames.size());
		vec[packet->bCategory] = info;
		m_PlayerCacheInfos.emplace(packet->dwPID, std::move(vec));

		auto& vecInfo = m_PlayerCacheInfos[packet->dwPID][packet->bCategory];
		writeInfos(vecInfo.aRankingPlayerInfo.rank, vecInfo.aRankingPlayerInfo.value);
	}


	peer->EncodeHeader(HEADER_DG_NEW_RANKING, dwHandle, sizeof(TNewRankingDGPacket) + sizeof(TNewRankingDGRankingInfo));
	peer->Encode(&mainPacket, sizeof(TNewRankingDGPacket));
	peer->Encode(&sendPacket, sizeof(TNewRankingDGRankingInfo));
}
#endif

#ifdef ENABLE_CSHIELD
void CClientManager::QUERY_CSHIELD_LOAD(CPeer* peer, DWORD dwHandle, TPlayerLoadPacket* packet)
{
	CCShieldCache* c;
	TCShield* tab;
	if (c = GetCShieldCache(packet->player_id))
	{
		tab = c->Get();
		peer->EncodeHeader(HEADER_DG_CSHIELD_LOAD, dwHandle, sizeof(TCShield));
		peer->Encode(tab, sizeof(TCShield));
	}
	else
	{
		char szQuery[QUERY_MAX_LEN];
		snprintf(szQuery, sizeof(szQuery),
			"SELECT metin_count, boss_count "
		...
		CDBManager::instance().ReturnQuery(szQuery, QID_CSHIELD, peer->GetHandle(), new ClientHandleInfo(dwHandle, packet->player_id));
	}
}

void CClientManager::RESULT_CSHIELD_LOAD(CPeer* peer, MYSQL_RES* pRes, DWORD dwHandle)
{
	...
	str_to_number(tab.metinCount, row[col++]);
	str_to_number(tab.bossCount, row[col++]);
#ifdef ENABLE_CAPTCHA
	str_to_number(tab.captchaTime, row[col++]);
	str_to_number(tab.captchaCount, row[col++]);
#endif

	sys_log(0, "CSHIELD_LOAD");
	peer->EncodeHeader(HEADER_DG_CSHIELD_LOAD, dwHandle, sizeof(TCShield));
	peer->Encode(&tab, sizeof(TCShield));
}
#endif
