...
namespace offlineshop
{
	...
	*/
	...
	bool CShopCache::ExtendShopTime(DWORD dwOwnerID, DWORD dwTime)
	{
		...
		TShopCacheInfo& rShop = it->second;
		rShop.dwDuration = rShop.dwDuration + dwTime;

		std::string query = CreateShopCacheUpdateShopTimeQuery(dwOwnerID, dwTime);
		CDBManager::instance().ReturnQuery(query.c_str(), QID_OFFLINESHOP_SHOP_EXTEND_TIME, 0, NULL);
		return true;
	}
#endif
	...
	{
		TSafeboxCacheInfo* pSafebox = NULL;
	...
	CSafeboxCache::TSafeboxCacheInfo* CSafeboxCache::CreateSafebox(DWORD dwOwnerID)
	{
		...
		CSafeboxCache::TSafeboxCacheInfo* pInfo=NULL;
		Get(dwOwnerID, &pInfo);
	...
	CSafeboxCache::TSafeboxCacheInfo* CSafeboxCache::LoadSafebox(DWORD dwPID)
	{
		...
		
		{
			...
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				str_to_number(item.iEvolution, row[col++]);
#endif
				//patch 08-03-2020
				BYTE exp = 0;
	...
	bool COfferCache::CancelOffer(DWORD dwOfferID, DWORD dwOwnerID) //offlineshop-updated 04/08/19 reminder
	{
		TOfferCacheInfo* pOffer = NULL;
		if (!Get(dwOfferID, &pOffer))
			return false;
...
#endif
#ifdef ENABLE_WEAPON_EVOLUTION_SYSTEM
		len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", rItem.item.iEvolution);
#endif
		//patch 08-03-2020
...
#endif
#ifdef ENABLE_WEAPON_EVOLUTION_SYSTEM
		len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", item.iEvolution);
#endif
		//patch 08-03-2020
...
#endif
#ifdef ENABLE_WEAPON_EVOLUTION_SYSTEM
		len += snprintf(szQuery + len, sizeof(szQuery)-len, " , %u ",auction.item.iEvolution);
#endif
		//patch 08-03-2020
