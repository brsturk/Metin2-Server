...
#ifdef ENABLE_MOUNT_CHECK_FIX
	int iPulse = thecore_pulse(); 
	if (iPulse - ch->GetMallLoadTime() < passes_per_sec * 1)
	{
...
// HARITA_SERBEST_MOD_KAPALI
	if (ch->GetMapIndex() == 325 || ch->GetMapIndex() == 327)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;2085]");
		return;
	}
// HARITA_SERBEST_MOD_KAPALI
...
ACMD(do_costume)
{
	...
	CItem* pHilal = ch->GetWear(WEAR_COSTUME_HILAL);
	CItem* pLolipop = ch->GetWear(WEAR_COSTUME_LOLIPOP);
	CItem* pGuc = ch->GetWear(WEAR_COSTUME_GUC);
	CItem* pSevgi = ch->GetWear(WEAR_COSTUME_SEVGI);
	CItem* pSevgi2 = ch->GetWear(WEAR_COSTUME_SEVGI2);
	CItem* pUyum = ch->GetWear(WEAR_COSTUME_UYUM);
#endif
	...
	if (pHilal)
	{
		const char* itemName = pHilal->GetName();
		ch->ChatPacket(CHAT_TYPE_INFO, "  HILAL : %s", itemName);

		if (pHilal->IsEquipped() && arg1[0] == 'm')
			ch->UnequipItem(pHilal);
	}
	
	if (pLolipop)
	{
		const char* itemName = pLolipop->GetName();
		ch->ChatPacket(CHAT_TYPE_INFO, "  LOLIPOP : %s", itemName);

		if (pLolipop->IsEquipped() && arg1[0] == 'm')
			ch->UnequipItem(pLolipop);
	}

	if (pGuc)
	{
		const char* itemName = pGuc->GetName();
		ch->ChatPacket(CHAT_TYPE_INFO, "  GUC : %s", itemName);

		if (pGuc->IsEquipped() && arg1[0] == 'm')
			ch->UnequipItem(pGuc);
	}

	if (pSevgi)
	{
		const char* itemName = pSevgi->GetName();
		ch->ChatPacket(CHAT_TYPE_INFO, "  SEVGI : %s", itemName);

		if (pSevgi->IsEquipped() && arg1[0] == 'm')
			ch->UnequipItem(pSevgi);
	}

	if (pSevgi2)
	{
		const char* itemName = pSevgi2->GetName();
		ch->ChatPacket(CHAT_TYPE_INFO, "  SEVGI2 : %s", itemName);

		if (pSevgi2->IsEquipped() && arg1[0] == 'm')
			ch->UnequipItem(pSevgi2);
	}

	if (pUyum)
	{
		const char* itemName = pUyum->GetName();
		ch->ChatPacket(CHAT_TYPE_INFO, "  UYUM : %s", itemName);

		if (pUyum->IsEquipped() && arg1[0] == 'm')
			ch->UnequipItem(pUyum);
	}
#endif
}
...
#endif
#ifdef ENABLE_COSTUME_RING_SYSTEM
				|| NULL==ch->GetWear(WEAR_COSTUME_HILAL)
				|| NULL==ch->GetWear(WEAR_COSTUME_LOLIPOP)
				|| NULL==ch->GetWear(WEAR_COSTUME_GUC)
				|| NULL==ch->GetWear(WEAR_COSTUME_SEVGI)
				|| NULL==ch->GetWear(WEAR_COSTUME_SEVGI2)
				|| NULL==ch->GetWear(WEAR_COSTUME_UYUM)
#endif
			)
			{
...
ACMD(do_event_manager)
{
	...
	else if (vecArgs[1] == "remove")
	{
		...
		if (vecArgs.size() < 3) {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("event indexi yanlis!!"));
			return;
		}
		...
		if (CHARACTER_MANAGER::Instance().CloseEventManuel(removeIndex))
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kapatildi"));
		else
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Herhangi bir event yok!"));
	}
	else if (vecArgs[1] == "update")
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Eventler guncellendi!"));
	}
}
#endif
...
ACMD(do_give_biolog) {
	...
	bool isTime = false; bool isChance = false; bool isFast = false;
	str_to_number(isTime, arg1);  str_to_number(isChance, arg2); str_to_number(isFast, arg3);
	if (isFast) {
		CNewBiologManager::instance().GiveFastBiolog(ch, isTime, isChance);
	}
	else {
...
ACMD(do_split_items)
{
	...
	char arg1[256], arg2[256], arg3[256];
	two_arguments(argument, arg1, sizeof(arg1), arg2, sizeof(arg2));
	one_argument(argument + strlen(arg1) + strlen(arg2) + 2, arg3, sizeof(arg3));

	if (!*arg1 || !*arg2 || !*arg3)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Kullanim: /split <slot> <miktar> <baslangıc>");
		return;
	}
	...
	WORD sourceCell, splitCount, startSearch;
	str_to_number(sourceCell, arg1);
	str_to_number(splitCount, arg2);
	str_to_number(startSearch, arg3);

	LPITEM item = ch->GetInventoryItem(sourceCell);

	if (!item || item->GetCount() <= splitCount || splitCount == 0)
	{
	...
	int itemLeft = item->GetCount();

	for (int i = startSearch; i < INVENTORY_MAX_NUM && itemLeft > splitCount; ++i)
	{
		if (i == sourceCell) continue;

		if (ch->IsEmptyItemGrid(TItemPos(INVENTORY, i), itemSize))
		{
			if (ch->MoveItem(TItemPos(INVENTORY, sourceCell), TItemPos(INVENTORY, i), splitCount))
			{
				itemLeft -= splitCount;
			}
			else
...
ACMD(do_split_items)
{
	...
	if (!ch->CanWarp())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1946]");
		return;
	}
	...
	if (!item || item->GetCount() <= splitCount || splitCount == 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1947]");
		return;
	}
	...
	for (int i = startSearch; i < INVENTORY_MAX_NUM && itemLeft > splitCount; ++i)
	{
		...
		if (ch->IsEmptyItemGrid(TItemPos(INVENTORY, i), itemSize))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1948]");
				break;
			}
		}
	}
#if defined(__SPECIAL_INVENTORY_SYSTEM__)
	for (int i = startSearch; i < SKILL_BOOK_INVENTORY_SLOT_END && itemLeft > splitCount; ++i)
	{
		if (i == sourceCell) continue;
	...
	for (int i = startSearch; i < UPGRADE_ITEMS_INVENTORY_SLOT_END && itemLeft > splitCount; ++i)
	{
		if (i == sourceCell) continue;
	...
	for (int i = startSearch; i < STONE_INVENTORY_SLOT_END && itemLeft > splitCount; ++i)
	{
		if (i == sourceCell) continue;
	...
	for (int i = startSearch; i < GIFT_BOX_INVENTORY_SLOT_END && itemLeft > splitCount; ++i)
	{
		if (i == sourceCell) continue;
	...
	if (itemLeft <= splitCount)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1949]");
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1950]");
	}
}
#endif
...
ACMD(do_refresh_is)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "[LS;2100]");
}
#endif
